...
exports.normalizeMysqlSql = exports.pool = void 0;
...
const MYSQL_TO_PG_CONFLICT_TARGET = {
...
const normalizeFormat = (format) => Object.entries(MYSQL_FORMAT_TO_PG).reduce((result, [mysql, pg]) => result.split(mysql).join(pg), format)
    .replace(/T/g, '"T"');
...
const replaceFunction = (sql, fnName, replacer) => {
...
const quoteCamelCaseAliases = (sql) => sql.replace(/\bAS\s+([A-Za-z_][A-Za-z0-9_]*)\b/g, (match, alias) => {
    const upper = String(alias).toUpperCase();
    if (SQL_KEYWORDS_AFTER_AS.has(upper))
        return match;
    return `AS "${alias}"`;
});
const convertMysqlCastTypes = (sql) => sql
    .replace(/\bAS\s+CHAR\b/gi, "AS TEXT")
    .replace(/\bAS\s+SIGNED\b/gi, "AS INTEGER")
    .replace(/\bAS\s+UNSIGNED\b/gi, "AS INTEGER");
const quoteAliasReferences = (sql) => {
...
const normalizeOverQuotedIdentifiers = (sql) => {
    let result = "";
    let cursor = 0;
...
const convertInsertIgnore = (sql) => {
    if (!/^\s*INSERT\s+IGNORE\s+INTO\b/i.test(sql))
        return sql;
...
const convertGroupConcat = (sql) => replaceFunction(sql, "GROUP_CONCAT", (args) => {
    const raw = args.join(", ");
...
const convertOnDuplicateKey = (sql) => {
...
const convertMysqlDdl = (sql) => {
    ...
    if (alterIndex)
        return `CREATE INDEX IF NOT EXISTS ${alterIndex[2]} ON ${alterIndex[1]} (${alterIndex[3]})`;
    const alterKey = out.match(/^\s*ALTER\s+TABLE\s+([A-Za-z0-9_]+)\s+ADD\s+KEY\s+([A-Za-z0-9_]+)\s*\(([^)]*)\)\s*;?\s*$/i);
    if (alterKey)
        return `CREATE INDEX IF NOT EXISTS ${alterKey[2]} ON ${alterKey[1]} (${alterKey[3]})`;
    const alterUnique = out.match(/^\s*ALTER\s+TABLE\s+([A-Za-z0-9_]+)\s+ADD\s+UNIQUE\s+(?:(?:KEY|INDEX)\s+)?([A-Za-z0-9_]+)\s*\(([^)]*)\)\s*;?\s*$/i);
    if (alterUnique)
        return `CREATE UNIQUE INDEX IF NOT EXISTS ${alterUnique[2]} ON ${alterUnique[1]} (${alterUnique[3]})`;
    return out;
};
const normalizeMysqlSql = (sourceSql) => {
    ...
    sql = replaceFunction(sql, "TIMESTAMPDIFF", (args) => {
        ...
        if (unit === "YEAR")
            return `EXTRACT(YEAR FROM AGE(${args[2]}, ${args[1]}))::int`;
        if (unit === "MONTH")
            return `(EXTRACT(YEAR FROM AGE(${args[2]}, ${args[1]}))::int * 12 + EXTRACT(MONTH FROM AGE(${args[2]}, ${args[1]}))::int)`;
...
exports.normalizeMysqlSql = normalizeMysqlSql;
const addReturningIdIfNeeded = (sql) => {
    if (!/^\s*INSERT\b/i.test(sql) || /\bRETURNING\b/i.test(sql))
        return sql;
    return `${sql} RETURNING id`;
};
const isSelectLike = (sql) => /^\s*(SELECT|WITH|SHOW|DESCRIBE)\b/i.test(sql);
const runQuery = async (executor, sourceSql, params = []) => {
    ...
    if (/INFORMATION_SCHEMA\.TABLES/i.test(sourceSql)) {
        ...
        if (/TABLE_NAME\s*=\s*\?/i.test(sourceSql) && tableName) {
            const result = await executor.query(`SELECT table_name AS "TABLE_NAME", table_name AS "tableName"
        ...
        const result = await executor.query(`SELECT c.relname AS "TABLE_NAME",
    ...
    if (/INFORMATION_SCHEMA\.COLUMNS/i.test(sourceSql)) {
        ...
        if (/COUNT\s*\(\s*\*\s*\)/i.test(sourceSql)) {
            const result = await executor.query(`SELECT COUNT(*)::int AS "total"
         FROM information_schema.columns
         WHERE table_schema = current_schema()
           AND table_name = $1
           AND ($2::text IS NULL OR column_name = $2)`, [tableName, columnName || null]);
            return [result.rows, undefined];
        }
        const result = await executor.query(`SELECT column_name AS "COLUMN_NAME",
       ...
       LIMIT CASE WHEN $2::text IS NULL THEN NULL ELSE 1 END`, [tableName, columnName || null]);
        return [result.rows, undefined];
    }
    ...
    if (!selectLike)
        sql = addReturningIdIfNeeded(sql);
    let result;
    try {
