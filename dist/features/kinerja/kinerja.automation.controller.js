...
exports.getKinerjaAutomationLogs = exports.generateKinerjaOverdueNotifications = exports.generateKinerjaPeriodicTargetsBulk = exports.generateKinerjaIkiBulk = exports.syncKinerjaEvaluatorMappings = exports.deleteKinerjaEvaluatorMapping = exports.updateKinerjaEvaluatorMapping = exports.createKinerjaEvaluatorMapping = exports.getKinerjaEvaluatorMappings = void 0;
const database_1 = require("../../config/database");
const http_1 = require("../../shared/http");
const validation_1 = require("../../shared/validation");
let automationSchemaReady = false;
const MAPPING_STATUSES = ["aktif", "arsip"];
const MAPPING_SOURCES = ["manual", "sinkron_tim"];
const TARGET_PERIOD_TYPES = ["bulanan", "triwulan", "semester"];
...
const ensureMappingExists = async (mappingId) => {
    const [rows] = await database_1.pool.query(`SELECT id FROM kinerja_pemetaan_penilai WHERE id = ? LIMIT 1`, [mappingId]);
    if (!rows.length)
        (0, http_1.fail)("Pemetaan penilai tidak ditemukan", 404);
};
...
const normalizeMappingPayload = (body) => ({
    ...
    sourceType: (0, validation_1.ensureOneOf)((0, validation_1.readTrimmedString)(body.sourceType || "manual").toLowerCase(), MAPPING_SOURCES, "Sumber pemetaan"),
    note: (0, validation_1.readTrimmedString)(body.note),
    status: (0, validation_1.ensureOneOf)((0, validation_1.readTrimmedString)(body.status || "aktif").toLowerCase(), MAPPING_STATUSES, "Status pemetaan")
});
const buildMappingRecord = (row) => ({
    id: Number(row.id),
...
const writeAutomationLog = async (payload) => {
    await database_1.pool.query(`INSERT INTO kinerja_otomatisasi_log
...
const createNotificationIfMissing = async (payload) => {
    const [rows] = await database_1.pool.query(`SELECT id
     ...
     LIMIT 1`, [payload.employeeId, payload.referenceType, payload.referenceId]);
    if (rows.length)
        return false;
    await database_1.pool.query(`INSERT INTO kinerja_notifikasi
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, 'baru')`, [payload.employeeId, payload.type, payload.title, payload.content, payload.link, payload.referenceType, payload.referenceId]);
    return true;
};
...
const buildSlices = (startDateRaw, endDateRaw, type) => {
...
const splitTarget = (target, parts) => {
...
exports.getKinerjaEvaluatorMappings = (0, http_1.asyncHandler)(async (req, res) => {
    ...
    const [rows] = await database_1.pool.query(`SELECT m.id,
     ...
     ORDER BY p.tahun DESC, p.nama_periode ASC, pg.nama_lengkap ASC`, params);
    return (0, http_1.sendSuccess)(res, rows.map(buildMappingRecord));
});
exports.createKinerjaEvaluatorMapping = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureAutomationSchema();
    ...
    if (payload.penilaiPegawaiId)
        await ensureEmployeeExists(payload.penilaiPegawaiId);
    if (payload.reviewerPegawaiId)
        await ensureEmployeeExists(payload.reviewerPegawaiId);
    const [result] = await database_1.pool.query(`INSERT INTO kinerja_pemetaan_penilai
    ...
    return (0, http_1.sendSuccess)(res, { id: result.insertId }, "Pemetaan penilai berhasil ditambahkan", 201);
});
exports.updateKinerjaEvaluatorMapping = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureAutomationSchema();
    const id = (0, validation_1.readPositiveId)(req.params.id, "Pemetaan penilai");
    ...
    if (payload.reviewerPegawaiId)
        await ensureEmployeeExists(payload.reviewerPegawaiId);
    await database_1.pool.query(`UPDATE kinerja_pemetaan_penilai
    ...
    return (0, http_1.sendSuccess)(res, null, "Pemetaan penilai berhasil diperbarui");
});
exports.deleteKinerjaEvaluatorMapping = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureAutomationSchema();
    const id = (0, validation_1.readPositiveId)(req.params.id, "Pemetaan penilai");
    const [result] = await database_1.pool.query(`DELETE FROM kinerja_pemetaan_penilai WHERE id = ?`, [id]);
    if (!result.affectedRows)
        (0, http_1.fail)("Pemetaan penilai tidak ditemukan", 404);
    return (0, http_1.sendSuccess)(res, null, "Pemetaan penilai berhasil dihapus");
});
exports.syncKinerjaEvaluatorMappings = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureAutomationSchema();
    ...
    const replaceManual = (0, validation_1.readBoolean)(req.body?.replaceManual, false);
    ...
    if (teamId)
        params.push(teamId);
    const [rows] = await database_1.pool.query(`SELECT tk.id AS teamId,
     ...
     ${teamFilter}
     ORDER BY tk.nama_tim ASC, ta.pegawai_id ASC`, params);
    const [existingRows] = await database_1.pool.query(`SELECT id, pegawai_id AS employeeId, sumber_mapping AS sourceType
     FROM kinerja_pemetaan_penilai
     WHERE periode_id = ?`, [periodId]);
    const existingByEmployee = new Map(existingRows.map((row) => [Number(row.employeeId), { id: Number(row.id), sourceType: String(row.sourceType || "manual") }]));
    let created = 0;
    let updated = 0;
    let skipped = 0;
    for (const row of rows) {
        ...
        if (current) {
         ...
         WHERE id = ?`, [Number(row.teamId), leaderId, current.id]);
            updated += 1;
        }
        else {
            const [result] = await database_1.pool.query(`INSERT INTO kinerja_pemetaan_penilai
          (periode_id, pegawai_id, tim_kerja_id, penilai_pegawai_id, reviewer_pegawai_id, sumber_mapping, status)
         VALUES (?, ?, ?, ?, NULL, 'sinkron_tim', 'aktif')`, [periodId, employeeId, Number(row.teamId), leaderId]);
    ...
    return (0, http_1.sendSuccess)(res, result, "Sinkronisasi pemetaan penilai selesai");
});
exports.generateKinerjaIkiBulk = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureAutomationSchema();
    const periodId = (0, validation_1.readPositiveId)(req.body?.periodeId, "Periode kinerja");
    const teamId = readOptionalPositiveId(req.body?.timKerjaId, "Tim kerja");
    const replaceExisting = (0, validation_1.readBoolean)(req.body?.replaceExisting, false);
    await ensurePeriodExists(periodId);
    if (teamId)
        await ensureTeamExists(teamId);
    const [teamRows] = await database_1.pool.query(`SELECT tk.id AS teamId, tk.ketua_pegawai_id AS leaderId, ta.pegawai_id AS memberId
     FROM kinerja_tim_kerja tk
    ...
    for (const row of teamRows) {
        ...
        const leaderId = row.leaderId == null ? null : Number(row.leaderId);
        if (leaderId && !list.includes(leaderId))
            list.push(leaderId);
        if (memberId && !list.includes(memberId))
            list.push(memberId);
    }
    ...
    const [rows] = await database_1.pool.query(`SELECT c.id,
     ...
     ORDER BY c.tim_kerja_id ASC, i.nama ASC`, params);
    let generated = 0;
    let updated = 0;
    let skipped = 0;
    ...
    for (const row of rows) {
        ...
        for (const employeeId of employeeCandidates) {
         ...
         LIMIT 1`, [periodId, employeeId, Number(row.indicatorId)]);
           ...
           WHERE id = ?`, [
                ...
                await database_1.pool.query(`INSERT INTO kinerja_iki_pegawai
    ...
    return (0, http_1.sendSuccess)(res, result, "Generate IKI massal selesai");
});
exports.generateKinerjaPeriodicTargetsBulk = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureAutomationSchema();
    const periodId = (0, validation_1.readPositiveId)(req.body?.periodeId, "Periode kinerja");
    const targetType = (0, validation_1.ensureOneOf)((0, validation_1.readTrimmedString)(req.body?.targetType || req.body?.jenisPeriode).toLowerCase(), TARGET_PERIOD_TYPES, "Jenis target periodik");
    const replaceExisting = (0, validation_1.readBoolean)(req.body?.replaceExisting, false);
    await ensurePeriodExists(periodId);
    const [[period]] = await database_1.pool.query(`SELECT tanggal_mulai AS startDate, tanggal_selesai AS endDate
     FROM kinerja_periode
     WHERE id = ? LIMIT 1`, [periodId]);
    if (!period)
        (0, http_1.fail)("Periode kinerja tidak ditemukan", 404);
    const slices = buildSlices(String(period.startDate), String(period.endDate), targetType);
    if (!slices.length)
        (0, http_1.fail)("Periode kinerja belum memiliki rentang tanggal yang valid", 400);
    const [ikiRows] = await database_1.pool.query(`SELECT id, nama_iki AS ikiName, target
     FROM kinerja_iki_pegawai
     WHERE periode_id = ?
     ORDER BY nama_iki ASC`, [periodId]);
    let generatedForIki = 0;
    let skippedIki = 0;
    let generatedTargets = 0;
    for (const row of ikiRows) {
        const ikiId = Number(row.id);
        const [existingRows] = await database_1.pool.query(`SELECT id FROM kinerja_target_periodik WHERE iki_pegawai_id = ? LIMIT 1`, [ikiId]);
        if (existingRows.length && !replaceExisting) {
            skippedIki += 1;
            continue;
        }
        if (existingRows.length && replaceExisting) {
            await database_1.pool.query(`DELETE FROM kinerja_target_periodik WHERE iki_pegawai_id = ?`, [ikiId]);
        }
    ...
    return (0, http_1.sendSuccess)(res, result, "Generate target periodik massal selesai");
});
exports.generateKinerjaOverdueNotifications = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureAutomationSchema();
    const periodId = (0, validation_1.readPositiveId)(req.body?.periodeId, "Periode kinerja");
    await ensurePeriodExists(periodId);
    const [assignmentRows] = await database_1.pool.query(`SELECT a.id,
     ...
     WHERE a.periode_id = ?
       ...
       AND a.status IN ('draft','berjalan','tertunda')`, [periodId]);
    const [targetRows] = await database_1.pool.query(`SELECT tp.id,
    ...
    return (0, http_1.sendSuccess)(res, result, "Generate notifikasi keterlambatan selesai");
});
exports.getKinerjaAutomationLogs = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureAutomationSchema();
    const periodId = readOptionalPositiveId(req.query.periodeId, "Periode kinerja");
    const conditions = ["1 = 1"];
    const params = [];
    ...
    const [rows] = await database_1.pool.query(`SELECT l.id,
     ...
     LIMIT 100`, params);
    return (0, http_1.sendSuccess)(res, rows.map(buildLogRecord));
});
