...
exports.finalizeIndicatorRealization = exports.verifyIndicatorRealization = exports.submitIndicatorRealization = exports.deleteIndicatorRealization = exports.updateIndicatorRealization = exports.createIndicatorRealization = exports.getIndicatorRealizations = exports.verifyActivityEvidence = exports.deleteActivityEvidence = exports.updateActivityEvidence = exports.createActivityEvidence = exports.getActivityEvidences = exports.deleteOperationalActivity = exports.pauseActiveTimekeeperActivity = exports.getOperationalActivityHistory = exports.updateOperationalActivity = exports.createOperationalActivity = exports.getOperationalActivities = exports.deleteOperationalAssignment = exports.updateOperationalAssignment = exports.createOperationalAssignment = exports.createTimekeeperAdditionalAssignment = exports.getOperationalAssignments = exports.ensureOperationalSchema = void 0;
...
const getKegiatanNameColumn = async () => {
    ...
    const columnNames = new Set(rows.map((row) => String(row.COLUMN_NAME || row.column_name || row.columnName || "").toLowerCase()));
    kegiatanNameColumnCache = columnNames.has("nama") ? "nama" : "nama_kegiatan";
    return kegiatanNameColumnCache;
};
const TASK_TYPES = ["individu", "tim", "lintas_tim", "lapangan", "pengolahan", "pst"];
const TASK_PRIORITIES = ["rendah", "sedang", "tinggi"];
const TASK_STATUSES = ["draft", "berjalan", "selesai", "tertunda"];
const ADMIN_LOGBOOK_STATUSES = ["draft", "berjalan", "jeda", "dijeda", "selesai", "dikirim", "disetujui", "revisi"];
const ACTIVITY_STATUSES = ["berjalan", "jeda", "selesai"];
...
const REALIZATION_STATUSES = ["draft", "submitted", "verified", "corrected", "finalized"];
...
const ACTIVE_TIMEKEEPER_STATUSES = ["berjalan", "jeda"];
...
const ensureSingleRunningActivity = async (employeeId, excludeActivityId = null) => {
    ...
    const [rows] = await database_1.pool.query(`SELECT id
     ...
     FOR UPDATE`, params);
    if (rows.length) {
        (0, http_1.fail)("Pegawai ini masih memiliki aktivitas berjalan. Jeda atau selesaikan aktivitas tersebut sebelum memulai aktivitas lain.", 409);
    }
};
...
const getActivityHistoryAction = (oldStatus, newStatus) => {
...
const writeActivityHistory = async (connection, payload) => {
    await connection.query(`INSERT INTO kinerja_activity_histories
...
const writeActivityFieldHistory = async (connection, payload) => {
...
const buildActivityHistoryRecord = (row) => ({
    id: Number(row.id),
...
const ensureEnumColumnIncludes = async (tableName, columnName, requiredValues, alterSql) => {
    const [rows] = await database_1.pool.query(`SELECT COLUMN_TYPE
     FROM INFORMATION_SCHEMA.COLUMNS
     WHERE TABLE_SCHEMA = DATABASE()
       AND TABLE_NAME = ?
       AND COLUMN_NAME = ?
...
const ensureTeamLeaderAccess = async (employeeId, teamId) => {
    if (!employeeId || !teamId) {
        (0, http_1.fail)("Ketua tim hanya dapat mengelola penugasan pada timnya", 403);
    }
    const [rows] = await database_1.pool.query(`SELECT id
     FROM kinerja_tim_kerja
     WHERE id = ?
       AND ketua_pegawai_id = ?
     LIMIT 1`, [teamId, employeeId]);
    if (!rows.length) {
        (0, http_1.fail)("Ketua tim hanya dapat mengelola penugasan pada timnya", 403);
    }
};
const ensureRecordExists = async (tableName, id, label) => {
    ...
    const [rows] = await database_1.pool.query(`SELECT id FROM ${tableName} WHERE id = ? LIMIT 1`, [numericId]);
    if (!rows.length)
...
const toDateOnlyTime = (dateValue) => new Date(`${dateValue}T00:00:00`).getTime();
...
const calculateAutomaticAssignmentStatus = (startDate, endDate) => {
...
const calculateAchievement = (target, realization) => {
...
const syncAssignmentProgressByRealization = async (assignmentId) => {
    if (!assignmentId)
        return;
    const [assignmentRows] = await database_1.pool.query(`SELECT output_target AS outputTarget
    ...
    const [realizationRows] = await database_1.pool.query(`SELECT COALESCE(SUM(COALESCE(l.volume, 0)), 0) AS totalRealization
     FROM kinerja_logbook l
     WHERE l.assignment_id = ?
       AND (
         ${(0, kinerja_timer_helper_1.activityStatusSql)('l')} IN ('berjalan', 'jeda', 'selesai')
...
const ensureOperationalSchema = async () => {
    ...
    await ensureEnumColumnIncludes("kinerja_logbook", "status", ["draft", "berjalan", "jeda", "dijeda", "selesai", "dikirim", "disetujui", "revisi"], "ALTER TABLE kinerja_logbook MODIFY status ENUM('draft','berjalan','jeda','dijeda','selesai','dikirim','disetujui','revisi') NOT NULL DEFAULT 'draft'");
...
exports.ensureOperationalSchema = ensureOperationalSchema;
const normalizeTaskPayload = (body) => {
    ...
    return {
        ...
        taskType: (0, validation_1.ensureOneOf)((0, validation_1.readTrimmedString)(body.taskType || "individu").toLowerCase(), TASK_TYPES, "Jenis penugasan"),
        title: (0, validation_1.ensureRequired)((0, validation_1.readTrimmedString)(body.title), "Judul penugasan wajib diisi"),
        startDate,
        endDate,
        priority: (0, validation_1.ensureOneOf)((0, validation_1.readTrimmedString)(body.priority || "sedang").toLowerCase(), TASK_PRIORITIES, "Prioritas"),
        outputTarget: (0, validation_1.readTrimmedString)(body.outputTarget),
        workRegion: (0, validation_1.readTrimmedString)(body.workRegion),
...
const resolveAssignmentActivityDefaults = async (activityId) => {
    if (!activityId) {
        return { indicatorId: null, teamId: null };
    }
    const [rows] = await database_1.pool.query(`SELECT kg.indikator_kinerja_id AS indicatorId,
...
const resolveLogbookAssignmentDefaults = async (assignmentId) => {
    if (!assignmentId) {
        return {
            employeeId: null,
            periodeId: null,
    ...
    const [rows] = await database_1.pool.query(`SELECT pegawai_id AS employeeId,
...
const normalizeActivityPayload = (body) => {
    const rawStatus = (0, kinerja_timer_helper_1.normalizeActivityStatusValue)(body.activityStatus ?? body.statusAktivitas ?? body.status ?? "draft");
    const isRealtimeStatus = ACTIVITY_STATUSES.includes(rawStatus);
    const adminStatus = (0, validation_1.ensureOneOf)(rawStatus, ADMIN_LOGBOOK_STATUSES, "Status logbook");
    const activityStatus = isRealtimeStatus
        ? (0, validation_1.ensureOneOf)(rawStatus, ACTIVITY_STATUSES, "Status aktivitas")
        : null;
    return {
        ...
        activityDate: body.activityDate ? (0, validation_1.readDateString)(body.activityDate, "Tanggal aktivitas") : getTodayDateOnly(),
        ...
        durationMinutes: body.durationMinutes === undefined || body.durationMinutes === null || body.durationMinutes === ""
            ? null
            : (0, validation_1.readIntegerInRange)(body.durationMinutes, 0, 1000000, "Durasi"),
        output: (0, validation_1.readTrimmedString)(body.output),
        status: adminStatus,
        activityStatus
    };
};
const normalizeEvidencePayload = (body) => ({
    activityId: (0, validation_1.readPositiveId)(body.activityId, "Aktivitas"),
    evidenceType: (0, validation_1.ensureOneOf)((0, validation_1.readTrimmedString)(body.evidenceType || "link").toLowerCase(), EVIDENCE_TYPES, "Jenis bukti"),
    fileName: (0, validation_1.ensureRequired)((0, validation_1.readTrimmedString)(body.fileName), "Nama bukti wajib diisi"),
    evidenceUrl: (0, validation_1.readTrimmedString)(body.evidenceUrl),
    note: (0, validation_1.readTrimmedString)(body.note),
    verificationStatus: (0, validation_1.ensureOneOf)((0, validation_1.readTrimmedString)(body.verificationStatus || "uploaded").toLowerCase(), EVIDENCE_VERIFICATION_STATUSES, "Status verifikasi")
});
const normalizeRealizationPayload = (body) => {
    ...
    return {
        ...
        periodLabel: (0, validation_1.readTrimmedString)(body.periodLabel),
        target,
        realization,
        achievement,
        verificationScore: readOptionalNumber(body.verificationScore, "Nilai verifikasi"),
        reportDate: (0, validation_1.readDateString)(body.reportDate, "Tanggal lapor"),
        employeeNote: (0, validation_1.readTrimmedString)(body.employeeNote),
        verifierNote: (0, validation_1.readTrimmedString)(body.verifierNote),
        status: (0, validation_1.ensureOneOf)((0, validation_1.readTrimmedString)(body.status || "draft").toLowerCase(), REALIZATION_STATUSES, "Status realisasi")
    };
};
const buildTaskRecord = (row) => ({
    id: Number(row.id),
...
const buildRealizationRecord = (row) => ({
    id: Number(row.id),
...
exports.getOperationalAssignments = (0, http_1.asyncHandler)(async (req, res) => {
    await (0, exports.ensureOperationalSchema)();
    ...
    if (req.query.periodId) {
        ...
        params.push((0, validation_1.readPositiveId)(req.query.periodId, "Periode kinerja"));
    }
    if (req.query.indicatorId) {
        conditions.push(`a.indikator_kinerja_id = ?`);
        params.push((0, validation_1.readPositiveId)(req.query.indicatorId, "Indikator kinerja"));
    }
    if (req.query.activityId) {
        conditions.push(`a.kegiatan_id = ?`);
        params.push((0, validation_1.readPositiveId)(req.query.activityId, "Kegiatan"));
    }
    if (req.query.status) {
        conditions.push(`(${automaticAssignmentStatusSql}) = ?`);
        params.push((0, validation_1.ensureOneOf)((0, validation_1.readTrimmedString)(req.query.status).toLowerCase(), TASK_STATUSES, "Status penugasan"));
    }
    if (req.query.timekeeperActive) {
        const activeOnly = ["1", "true", "ya", "aktif", "active"].includes((0, validation_1.readTrimmedString)(req.query.timekeeperActive).toLowerCase());
        if (activeOnly) {
    ...
    const [rows] = await database_1.pool.query(`SELECT a.id,
     ...
     ${whereSql}
     ORDER BY a.target_mulai DESC, a.dibuat_pada DESC, a.id DESC`, params);
    return (0, http_1.sendSuccess)(res, rows.map(buildTaskRecord));
});
const ensureAssignmentMutationAllowed = (req) => {
    if (req.user?.role === "pegawai") {
        (0, http_1.fail)("Role pegawai hanya dapat melihat daftar penugasan yang ditugaskan kepadanya", 403);
    }
};
exports.createTimekeeperAdditionalAssignment = (0, http_1.asyncHandler)(async (req, res) => {
    await (0, exports.ensureOperationalSchema)();
    const currentUser = req.user;
    ...
    const activity = (0, validation_1.ensureRequired)((0, validation_1.readTrimmedString)((req.body || {}).activity), "Uraian kegiatan wajib diisi");
    ...
    const [result] = await database_1.pool.query(`INSERT INTO kinerja_assignment
    ...
    return (0, http_1.sendSuccess)(res, { id: result.insertId, reused: false }, "Penugasan Tambahan berhasil dibuat", 201);
});
exports.createOperationalAssignment = (0, http_1.asyncHandler)(async (req, res) => {
    ...
    if (payload.activityId)
        await ensureRecordExists("kegiatan_indikator_kinerja", payload.activityId, "Kegiatan");
    ...
    if (resolvedTeamId)
        await ensureRecordExists("kinerja_tim_kerja", resolvedTeamId, "Tim kerja");
    if (resolvedIndicatorId)
        await ensureRecordExists("indikator_kinerja", resolvedIndicatorId, "Indikator kinerja");
...
exports.updateOperationalAssignment = (0, http_1.asyncHandler)(async (req, res) => {
    ensureAssignmentMutationAllowed(req);
    await (0, exports.ensureOperationalSchema)();
...
exports.deleteOperationalAssignment = (0, http_1.asyncHandler)(async (req, res) => {
    ensureAssignmentMutationAllowed(req);
    await (0, exports.ensureOperationalSchema)();
    const id = (0, validation_1.readPositiveId)(req.params.id, "Penugasan");
...
exports.getOperationalActivities = (0, http_1.asyncHandler)(async (req, res) => {
    await (0, exports.ensureOperationalSchema)();
    const kegiatanNameColumn = await getKegiatanNameColumn();
    const conditions = [];
    const params = [];
    ...
    if (req.query.id) {
        ...
        params.push((0, validation_1.readPositiveId)(req.query.id, "Aktivitas"));
    }
    if (req.query.activeTimekeeper) {
        const activeOnly = ["1", "true", "ya", "aktif", "active"].includes((0, validation_1.readTrimmedString)(req.query.activeTimekeeper).toLowerCase());
        if (activeOnly) {
            conditions.push(`${(0, kinerja_timer_helper_1.activityStatusSql)('l')} IN ('berjalan', 'jeda')`);
        }
    }
    ...
    if (req.query.status) {
        ...
        params.push((0, validation_1.ensureOneOf)((0, kinerja_timer_helper_1.normalizeActivityStatusValue)(req.query.status), ACTIVITY_STATUSES, "Status aktivitas"));
    }
    if (req.query.month) {
    ...
    const [rows] = await database_1.pool.query(`SELECT l.id,
     ...
     ${whereSql}
     ORDER BY l.tanggal_kegiatan DESC, l.dibuat_pada DESC, l.id DESC`, params);
    return (0, http_1.sendSuccess)(res, rows.map(buildActivityRecord));
});
exports.createOperationalActivity = (0, http_1.asyncHandler)(async (req, res) => {
    await (0, exports.ensureOperationalSchema)();
    ...
    if (currentUser?.role === "pegawai" && assignmentDefaults.employeeId && assignmentDefaults.employeeId !== resolvedEmployeeId) {
        (0, http_1.fail)("Pegawai hanya dapat mencatat aktivitas dari penugasan miliknya sendiri", 403);
    }
    await ensureRecordExists("pegawai", resolvedEmployeeId, "Pegawai");
    if (resolvedPeriodeId)
        await ensureRecordExists("kinerja_periode", resolvedPeriodeId, "Periode kinerja");
    if (payload.assignmentId)
        await ensureRecordExists("kinerja_assignment", payload.assignmentId, "Penugasan");
    if (resolvedTeamId)
        await ensureRecordExists("kinerja_tim_kerja", resolvedTeamId, "Tim kerja");
    if (resolvedIndicatorId)
        await ensureRecordExists("indikator_kinerja", resolvedIndicatorId, "Indikator kinerja");
    if (resolvedActivityId)
        await ensureRecordExists("kegiatan_indikator_kinerja", resolvedActivityId, "Kegiatan");
    if (payload.categoryId)
        await ensureRecordExists("kinerja_kategori_aktivitas", payload.categoryId, "Kategori aktivitas");
    if (payload.unitId)
        await ensureRecordExists("kinerja_satuan", payload.unitId, "Satuan");
    const connection = await database_1.pool.getConnection();
    try {
        await connection.beginTransaction();
        if (payload.activityStatus === "berjalan") {
            const [runningRows] = await connection.query(`SELECT id
         FROM kinerja_logbook
         WHERE pegawai_id = ?
           AND COALESCE(status_aktivitas, CASE WHEN status = 'dijeda' THEN 'jeda' ELSE status END) = 'berjalan'
         LIMIT 1
         FOR UPDATE`, [resolvedEmployeeId]);
       ...
       SET durasi_menit = ${(0, kinerja_timer_helper_1.getTimerDurationMinutesSql)('kinerja_logbook')}
...
exports.updateOperationalActivity = (0, http_1.asyncHandler)(async (req, res) => {
    await (0, exports.ensureOperationalSchema)();
    const id = (0, validation_1.readPositiveId)(req.params.id, "Aktivitas");
    const payload = normalizeActivityPayload(req.body || {});
    const currentUser = req.user;
    const assignmentDefaults = await resolveLogbookAssignmentDefaults(payload.assignmentId);
    const loginEmployeeId = Number(currentUser?.employeeId || 0);
    ...
    const resolvedActivityId = assignmentDefaults.activityId || payload.activityId;
    if (currentUser?.role === "pegawai" && assignmentDefaults.employeeId && assignmentDefaults.employeeId !== resolvedEmployeeId) {
        (0, http_1.fail)("Pegawai hanya dapat memperbarui aktivitas dari penugasan miliknya sendiri", 403);
    }
    await ensureRecordExists("pegawai", resolvedEmployeeId, "Pegawai");
    if (resolvedPeriodeId)
        await ensureRecordExists("kinerja_periode", resolvedPeriodeId, "Periode kinerja");
    ...
    try {
       ...
       FOR UPDATE`, [id]);
        ...
        const previousActivityStatus = (0, kinerja_timer_helper_1.normalizeActivityStatusValue)(existingRows[0]?.activityStatus || existingRows[0]?.status || "");
        if (previousActivityStatus === "selesai" && payload.activityStatus && payload.activityStatus !== "selesai") {
            (0, http_1.fail)("Aktivitas yang sudah selesai tidak dapat dijalankan atau dijeda kembali. Gunakan fitur edit data selesai bila perubahan administratif memang diperlukan.", 409);
        }
        if (payload.activityStatus === "berjalan") {
            const [runningRows] = await connection.query(`SELECT id
         FROM kinerja_logbook
         WHERE pegawai_id = ?
           ...
           AND id <> ?
         LIMIT 1
         FOR UPDATE`, [resolvedEmployeeId, id]);
            if (runningRows.length) {
                (0, http_1.fail)("Pegawai ini masih memiliki aktivitas berjalan. Jeda atau selesaikan aktivitas tersebut sebelum memulai aktivitas lain.", 409);
            }
        }
       ...
       SET pegawai_id = ?,
           ...
           total_paused_seconds = CASE
             WHEN ${(0, kinerja_timer_helper_1.activityStatusSql)('kinerja_logbook')} = 'jeda' AND ? = 'berjalan' AND paused_at IS NOT NULL THEN COALESCE(total_paused_seconds, 0) + GREATEST(0, TIMESTAMPDIFF(SECOND, paused_at, NOW()))
           ...
           END,
           finished_at = CASE
             WHEN ? = 'selesai' THEN CASE WHEN ${(0, kinerja_timer_helper_1.activityStatusSql)('kinerja_logbook')} = 'jeda' THEN COALESCE(paused_at, NOW()) ELSE NOW() END
       ...
       WHERE id = ?`, [id]);
        ...
        return (0, http_1.sendSuccess)(res, { id }, "Aktivitas berhasil diperbarui");
    }
    catch (error) {
        await connection.rollback();
        throw error;
...
exports.getOperationalActivityHistory = (0, http_1.asyncHandler)(async (req, res) => {
    await (0, exports.ensureOperationalSchema)();
    const id = (0, validation_1.readPositiveId)(req.params.id, "Aktivitas");
    const kegiatanNameColumn = await getKegiatanNameColumn();
    const currentUser = req.user;
    ...
    const [rows] = await database_1.pool.query(`SELECT l.id,
     ...
     LIMIT 1`, params);
    if (!rows.length) {
        (0, http_1.fail)("Aktivitas tidak ditemukan atau tidak dapat diakses", 404);
    }
    const [historyRows] = await database_1.pool.query(`SELECT id,
...
exports.pauseActiveTimekeeperActivity = (0, http_1.asyncHandler)(async (req, res) => {
    await (0, exports.ensureOperationalSchema)();
    ...
    try {
       ...
       FROM kinerja_logbook
       WHERE pegawai_id = ?
         AND ${(0, kinerja_timer_helper_1.activityStatusSql)('kinerja_logbook')} = 'berjalan'
       FOR UPDATE`, [employeeId]);
        if (!runningRows.length) {
            await connection.commit();
            return (0, http_1.sendSuccess)(res, { pausedCount: 0 }, "Tidak ada aktivitas berjalan yang perlu dijeda");
        }
        await connection.query(`UPDATE kinerja_logbook
       SET paused_at = COALESCE(paused_at, NOW()),
           ...
           diperbarui_pada = NOW()
       WHERE pegawai_id = ?
         AND ${(0, kinerja_timer_helper_1.activityStatusSql)('kinerja_logbook')} = 'berjalan'`, [employeeId]);
        await connection.query(`UPDATE kinerja_logbook
       SET durasi_menit = ${(0, kinerja_timer_helper_1.getTimerDurationMinutesSql)('kinerja_logbook')}
       WHERE id IN (${runningRows.map(() => "?").join(",")})`, runningRows.map((row) => Number(row.id)));
        ...
        await connection.commit();
        return (0, http_1.sendSuccess)(res, { pausedCount: runningRows.length }, "Aktivitas berjalan berhasil dijeda");
    }
    catch (error) {
        await connection.rollback();
        throw error;
...
exports.deleteOperationalActivity = (0, http_1.asyncHandler)(async (req, res) => {
    await (0, exports.ensureOperationalSchema)();
    const id = (0, validation_1.readPositiveId)(req.params.id, "Aktivitas");
    const [existingRows] = await database_1.pool.query(`SELECT assignment_id AS assignmentId FROM kinerja_logbook WHERE id = ? LIMIT 1`, [id]);
...
exports.getActivityEvidences = (0, http_1.asyncHandler)(async (req, res) => {
    await (0, exports.ensureOperationalSchema)();
    const conditions = [];
    const params = [];
    ...
    if (req.query.activityId) {
        ...
        params.push((0, validation_1.readPositiveId)(req.query.activityId, "Aktivitas"));
    }
    if (req.query.status) {
        conditions.push(`b.status_verifikasi = ?`);
        params.push((0, validation_1.ensureOneOf)((0, validation_1.readTrimmedString)(req.query.status).toLowerCase(), EVIDENCE_VERIFICATION_STATUSES, "Status verifikasi"));
    }
    if (req.query.search) {
        const keyword = `%${(0, validation_1.readTrimmedString)(req.query.search)}%`;
    ...
    const [rows] = await database_1.pool.query(`SELECT b.id,
     ...
     LEFT JOIN pegawai p ON p.id = l.pegawai_id
     ${whereSql}
     ORDER BY b.dibuat_pada DESC, b.id DESC`, params);
    const origin = `${req.protocol}://${req.get("host")}`;
    const data = rows.map((row) => buildEvidenceRecord({
        ...row,
        fileUrl: row.filePath ? `${origin}${String(row.filePath).startsWith('/') ? row.filePath : `/${row.filePath}`}` : ""
    }));
    return (0, http_1.sendSuccess)(res, data);
});
exports.createActivityEvidence = (0, http_1.asyncHandler)(async (req, res) => {
    await (0, exports.ensureOperationalSchema)();
    ...
    const [result] = await database_1.pool.query(`INSERT INTO kinerja_logbook_bukti
     ...
     VALUES (?, ?, ?, ?, ?, ?, 1, 0, 0)`, [payload.activityId, payload.evidenceType, payload.fileName, payload.evidenceUrl || null, payload.note || null, payload.verificationStatus]);
    return (0, http_1.sendSuccess)(res, { id: result.insertId }, "Bukti dukung berhasil ditambahkan", 201);
});
exports.updateActivityEvidence = (0, http_1.asyncHandler)(async (req, res) => {
    await (0, exports.ensureOperationalSchema)();
    ...
    await database_1.pool.query(`UPDATE kinerja_logbook_bukti
     SET logbook_id = ?,
         ...
         status_verifikasi = ?,
         diperbarui_pada = NOW()
     WHERE id = ?`, [payload.activityId, payload.evidenceType, payload.fileName, payload.evidenceUrl || null, payload.note || null, payload.verificationStatus, id]);
    return (0, http_1.sendSuccess)(res, null, "Bukti dukung berhasil diperbarui");
});
exports.deleteActivityEvidence = (0, http_1.asyncHandler)(async (req, res) => {
    await (0, exports.ensureOperationalSchema)();
    const id = (0, validation_1.readPositiveId)(req.params.id, "Bukti dukung");
    const [result] = await database_1.pool.query(`DELETE FROM kinerja_logbook_bukti WHERE id = ?`, [id]);
    if (!result.affectedRows)
        (0, http_1.fail)("Bukti dukung tidak ditemukan", 404);
    return (0, http_1.sendSuccess)(res, null, "Bukti dukung berhasil dihapus");
});
exports.verifyActivityEvidence = (0, http_1.asyncHandler)(async (req, res) => {
    await (0, exports.ensureOperationalSchema)();
    const id = (0, validation_1.readPositiveId)(req.params.id, "Bukti dukung");
    const status = (0, validation_1.ensureOneOf)((0, validation_1.readTrimmedString)((req.body || {}).verificationStatus || "verified").toLowerCase(), EVIDENCE_VERIFICATION_STATUSES, "Status verifikasi");
    const [result] = await database_1.pool.query(`UPDATE kinerja_logbook_bukti SET status_verifikasi = ?, diperbarui_pada = NOW() WHERE id = ?`, [status, id]);
    if (!result.affectedRows)
        (0, http_1.fail)("Bukti dukung tidak ditemukan", 404);
    return (0, http_1.sendSuccess)(res, null, "Status verifikasi bukti diperbarui");
});
exports.getIndicatorRealizations = (0, http_1.asyncHandler)(async (req, res) => {
    await (0, exports.ensureOperationalSchema)();
    const conditions = [];
    const params = [];
    ...
    if (req.query.status) {
        ...
        params.push((0, validation_1.ensureOneOf)((0, validation_1.readTrimmedString)(req.query.status).toLowerCase(), REALIZATION_STATUSES, "Status realisasi"));
    }
    if (req.query.search) {
        const keyword = `%${(0, validation_1.readTrimmedString)(req.query.search)}%`;
    ...
    const [rows] = await database_1.pool.query(`SELECT r.id,
     ...
     ${whereSql}
     ORDER BY r.tanggal_lapor DESC, r.dibuat_pada DESC, r.id DESC`, params);
    return (0, http_1.sendSuccess)(res, rows.map(buildRealizationRecord));
});
exports.createIndicatorRealization = (0, http_1.asyncHandler)(async (req, res) => {
    await (0, exports.ensureOperationalSchema)();
    ...
    if (payload.targetPeriodikId)
        await ensureRecordExists("kinerja_target_periodik", payload.targetPeriodikId, "Target periodik");
    const [result] = await database_1.pool.query(`INSERT INTO kinerja_realisasi_indikator
    ...
    return (0, http_1.sendSuccess)(res, { id: result.insertId }, "Realisasi indikator berhasil ditambahkan", 201);
});
exports.updateIndicatorRealization = (0, http_1.asyncHandler)(async (req, res) => {
    await (0, exports.ensureOperationalSchema)();
    const id = (0, validation_1.readPositiveId)(req.params.id, "Realisasi indikator");
    await ensureRecordExists("kinerja_realisasi_indikator", id, "Realisasi indikator");
    const payload = normalizeRealizationPayload(req.body || {});
    await ensureRecordExists("kinerja_iki_pegawai", payload.ikiPegawaiId, "IKI Pegawai");
    if (payload.targetPeriodikId)
        await ensureRecordExists("kinerja_target_periodik", payload.targetPeriodikId, "Target periodik");
    await database_1.pool.query(`UPDATE kinerja_realisasi_indikator
     SET iki_pegawai_id = ?,
         target_periodik_id = ?,
         periode_lapor = ?,
         target = ?,
    ...
    return (0, http_1.sendSuccess)(res, null, "Realisasi indikator berhasil diperbarui");
});
exports.deleteIndicatorRealization = (0, http_1.asyncHandler)(async (req, res) => {
    await (0, exports.ensureOperationalSchema)();
    const id = (0, validation_1.readPositiveId)(req.params.id, "Realisasi indikator");
    const [result] = await database_1.pool.query(`DELETE FROM kinerja_realisasi_indikator WHERE id = ?`, [id]);
    if (!result.affectedRows)
        (0, http_1.fail)("Realisasi indikator tidak ditemukan", 404);
    return (0, http_1.sendSuccess)(res, null, "Realisasi indikator berhasil dihapus");
});
const updateRealizationStatus = async (id, status, verificationScore, verifierNote) => {
    const [result] = await database_1.pool.query(`UPDATE kinerja_realisasi_indikator
     SET status = ?,
         nilai_verifikasi = COALESCE(?, nilai_verifikasi),
         catatan_verifikator = COALESCE(?, catatan_verifikator),
         diperbarui_pada = NOW()
     WHERE id = ?`, [status, verificationScore ?? null, verifierNote || null, id]);
    if (!result.affectedRows)
        (0, http_1.fail)("Realisasi indikator tidak ditemukan", 404);
};
exports.submitIndicatorRealization = (0, http_1.asyncHandler)(async (req, res) => {
    await (0, exports.ensureOperationalSchema)();
    await updateRealizationStatus((0, validation_1.readPositiveId)(req.params.id, "Realisasi indikator"), "submitted");
    return (0, http_1.sendSuccess)(res, null, "Realisasi indikator berhasil dikirim");
});
exports.verifyIndicatorRealization = (0, http_1.asyncHandler)(async (req, res) => {
    await (0, exports.ensureOperationalSchema)();
    const id = (0, validation_1.readPositiveId)(req.params.id, "Realisasi indikator");
    const verificationScore = readOptionalNumber((req.body || {}).verificationScore, "Nilai verifikasi");
    const verifierNote = (0, validation_1.readTrimmedString)((req.body || {}).verifierNote);
    await updateRealizationStatus(id, "verified", verificationScore, verifierNote);
    return (0, http_1.sendSuccess)(res, null, "Realisasi indikator berhasil diverifikasi");
});
exports.finalizeIndicatorRealization = (0, http_1.asyncHandler)(async (req, res) => {
    await (0, exports.ensureOperationalSchema)();
    await updateRealizationStatus((0, validation_1.readPositiveId)(req.params.id, "Realisasi indikator"), "finalized");
    return (0, http_1.sendSuccess)(res, null, "Realisasi indikator berhasil difinalkan");
});
