...
exports.closeKinerjaCalibration = exports.updateKinerjaCalibrationItem = exports.createKinerjaCalibrationItem = exports.updateKinerjaCalibration = exports.createKinerjaCalibration = exports.getKinerjaCalibrations = exports.finalizeKinerjaFinalEvaluation = exports.updateKinerjaFinalEvaluation = exports.generateKinerjaFinalEvaluations = exports.getKinerjaFinalEvaluations = exports.finalizeKinerjaMidyearEvaluation = exports.updateKinerjaMidyearEvaluation = exports.createKinerjaMidyearEvaluation = exports.getKinerjaMidyearEvaluations = exports.updateKinerjaPredicate = exports.createKinerjaPredicate = exports.getKinerjaPredicates = exports.updateKinerjaPolicyFormula = exports.createKinerjaPolicyFormula = exports.getKinerjaPolicyFormulas = exports.activateKinerjaPolicy = exports.updateKinerjaPolicy = exports.createKinerjaPolicy = exports.getKinerjaPolicies = void 0;
const database_1 = require("../../config/database");
const http_1 = require("../../shared/http");
const validation_1 = require("../../shared/validation");
let evaluationSchemaReady = false;
const POLICY_STATUSES = ["draft", "aktif", "nonaktif"];
const ARCHIVE_STATUSES = ["aktif", "arsip"];
const COMPONENTS = ["iku", "iki", "penugasan", "aktivitas", "perilaku"];
const ROUNDING_TYPES = ["normal", "ke_atas", "ke_bawah"];
const MIDYEAR_STATUSES = ["draft", "ditinjau", "final"];
const FINAL_STATUSES = ["draft", "generated", "reviewed", "calibrated", "finalized"];
const CALIBRATION_STATUSES = ["draft", "closed"];
...
const ensureColumnExists = async (tableName, columnName, definitionSql) => {
    const [rows] = await database_1.pool.query(`SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = ? AND COLUMN_NAME = ? LIMIT 1`, [tableName, columnName]);
    if (!rows.length) {
        await database_1.pool.query(`ALTER TABLE ${tableName} ADD COLUMN ${definitionSql}`);
    }
};
...
const ensurePolicyExists = async (policyId) => {
    const [rows] = await database_1.pool.query(`SELECT id FROM kinerja_kebijakan_penilaian WHERE id = ? LIMIT 1`, [policyId]);
    if (!rows.length)
        (0, http_1.fail)("Kebijakan penilaian tidak ditemukan", 404);
};
const ensureFinalEvaluationExists = async (evaluationId) => {
    const [rows] = await database_1.pool.query(`SELECT id FROM kinerja_evaluasi_akhir_tahun WHERE id = ? LIMIT 1`, [evaluationId]);
    if (!rows.length)
        (0, http_1.fail)("Evaluasi akhir tahun tidak ditemukan", 404);
};
const ensureMidyearEvaluationExists = async (evaluationId) => {
    const [rows] = await database_1.pool.query(`SELECT id FROM kinerja_evaluasi_tengah_tahun WHERE id = ? LIMIT 1`, [evaluationId]);
    if (!rows.length)
        (0, http_1.fail)("Evaluasi tengah tahun tidak ditemukan", 404);
};
const ensureCalibrationExists = async (calibrationId) => {
    const [rows] = await database_1.pool.query(`SELECT id FROM kinerja_kalibrasi WHERE id = ? LIMIT 1`, [calibrationId]);
    if (!rows.length)
        (0, http_1.fail)("Sesi kalibrasi tidak ditemukan", 404);
};
const ensureCalibrationItemExists = async (itemId) => {
    const [rows] = await database_1.pool.query(`SELECT id FROM kinerja_kalibrasi_item WHERE id = ? LIMIT 1`, [itemId]);
    if (!rows.length)
        (0, http_1.fail)("Item kalibrasi tidak ditemukan", 404);
};
...
const normalizePolicyPayload = (body) => ({
    periodeId: readOptionalPositiveId(body.periodeId, 'Periode kinerja'),
    policyName: (0, validation_1.ensureRequired)((0, validation_1.readTrimmedString)(body.policyName), 'Nama kebijakan wajib diisi'),
    version: (0, validation_1.ensureRequired)((0, validation_1.readTrimmedString)(body.version), 'Versi kebijakan wajib diisi'),
    effectiveDate: (0, validation_1.readDateString)(body.effectiveDate, 'Tanggal berlaku'),
    status: (0, validation_1.ensureOneOf)((0, validation_1.readTrimmedString)(body.status || 'draft').toLowerCase(), POLICY_STATUSES, 'Status kebijakan'),
    note: (0, validation_1.readTrimmedString)(body.note)
});
const normalizeFormulaPayload = (body) => ({
    policyId: (0, validation_1.readPositiveId)(body.policyId, 'Kebijakan penilaian'),
    component: (0, validation_1.ensureOneOf)((0, validation_1.readTrimmedString)(body.component || 'iki').toLowerCase(), COMPONENTS, 'Komponen formula'),
    jobType: (0, validation_1.ensureRequired)((0, validation_1.readTrimmedString)(body.jobType || 'umum'), 'Jenis jabatan wajib diisi'),
    weight: (0, validation_1.readNonNegativeNumber)(body.weight, 'Bobot', 0),
    ...
    roundingType: (0, validation_1.ensureOneOf)((0, validation_1.readTrimmedString)(body.roundingType || 'normal').toLowerCase(), ROUNDING_TYPES, 'Jenis pembulatan'),
    penalty: readOptionalNonNegativeNumber(body.penalty, 'Penalti'),
    bonus: readOptionalNonNegativeNumber(body.bonus, 'Bonus'),
    status: (0, validation_1.ensureOneOf)((0, validation_1.readTrimmedString)(body.status || 'aktif').toLowerCase(), ARCHIVE_STATUSES, 'Status formula')
});
const normalizePredicatePayload = (body) => {
    const minScore = (0, validation_1.readNonNegativeNumber)(body.minScore, 'Nilai minimum', 0);
    const maxScore = (0, validation_1.readNonNegativeNumber)(body.maxScore, 'Nilai maksimum', 0);
    if (maxScore < minScore)
        (0, http_1.fail)('Nilai maksimum tidak boleh lebih kecil dari nilai minimum', 400);
    return {
        policyId: (0, validation_1.readPositiveId)(body.policyId, 'Kebijakan penilaian'),
        minScore,
        maxScore,
        predicateName: (0, validation_1.ensureRequired)((0, validation_1.readTrimmedString)(body.predicateName), 'Nama predikat wajib diisi'),
        recommendation: (0, validation_1.readTrimmedString)(body.recommendation),
        status: (0, validation_1.ensureOneOf)((0, validation_1.readTrimmedString)(body.status || 'aktif').toLowerCase(), ARCHIVE_STATUSES, 'Status predikat')
    };
};
const normalizeMidyearPayload = (body, fallbackEvaluatorId) => ({
    ...
    achievementSummary: (0, validation_1.readTrimmedString)(body.achievementSummary),
    obstacles: (0, validation_1.readTrimmedString)(body.obstacles),
    improvementPlan: (0, validation_1.readTrimmedString)(body.improvementPlan),
    temporaryScore: readOptionalNonNegativeNumber(body.temporaryScore, 'Nilai sementara'),
    evaluatorNote: (0, validation_1.readTrimmedString)(body.evaluatorNote),
    evaluationDate: (0, validation_1.readDateString)(body.evaluationDate, 'Tanggal evaluasi'),
    status: (0, validation_1.ensureOneOf)((0, validation_1.readTrimmedString)(body.status || 'draft').toLowerCase(), MIDYEAR_STATUSES, 'Status evaluasi tengah tahun')
});
const normalizeFinalPayload = (body, fallbackEvaluatorId) => ({
    periodeId: (0, validation_1.readPositiveId)(body.periodeId, 'Periode kinerja'),
    employeeId: (0, validation_1.readPositiveId)(body.employeeId, 'Pegawai'),
    evaluatorId: readOptionalPositiveId(body.evaluatorId, 'Penilai') || fallbackEvaluatorId || 0,
    policyId: readOptionalPositiveId(body.policyId, 'Kebijakan penilaian'),
    ...
    predicate: (0, validation_1.readTrimmedString)(body.predicate),
    evaluatorNote: (0, validation_1.readTrimmedString)(body.evaluatorNote),
    evaluationDate: (0, validation_1.readDateString)(body.evaluationDate, 'Tanggal evaluasi'),
    status: (0, validation_1.ensureOneOf)((0, validation_1.readTrimmedString)(body.status || 'draft').toLowerCase(), FINAL_STATUSES, 'Status evaluasi akhir tahun')
});
const normalizeCalibrationPayload = (body, fallbackChairId) => ({
    periodeId: (0, validation_1.readPositiveId)(body.periodeId, 'Periode kinerja'),
    sessionName: (0, validation_1.ensureRequired)((0, validation_1.readTrimmedString)(body.sessionName), 'Nama sesi wajib diisi'),
    calibrationDate: (0, validation_1.readDateString)(body.calibrationDate, 'Tanggal kalibrasi'),
    chairpersonId: readOptionalPositiveId(body.chairpersonId, 'Ketua sesi') || fallbackChairId || 0,
    note: (0, validation_1.readTrimmedString)(body.note),
    status: (0, validation_1.ensureOneOf)((0, validation_1.readTrimmedString)(body.status || 'draft').toLowerCase(), CALIBRATION_STATUSES, 'Status kalibrasi')
});
const normalizeCalibrationItemPayload = (body, fallbackReviewerId) => ({
    evaluationId: (0, validation_1.readPositiveId)(body.evaluationId, 'Evaluasi akhir tahun'),
    employeeId: (0, validation_1.readPositiveId)(body.employeeId, 'Pegawai'),
...
const buildPredicateRecord = (row) => ({
    id: Number(row.id),
    policyId: Number(row.policyId),
    policyName: String(row.policyName || '-'),
...
const buildCalibrationItemRecord = (row) => ({
    id: Number(row.id),
...
const buildCalibrationRecord = (row, items) => ({
    id: Number(row.id),
    periodeId: Number(row.periodeId),
    periodeName: String(row.periodeName || '-'),
...
const resolvePredicateByPolicy = async (policyId, score) => {
    if (!policyId)
        return score >= 110 ? 'Istimewa' : score >= 90 ? 'Baik Sekali' : score >= 76 ? 'Baik' : score >= 60 ? 'Cukup' : 'Perlu Pembinaan';
    const [rows] = await database_1.pool.query(`SELECT nama_predikat AS predicateName FROM kinerja_kebijakan_predikat WHERE kebijakan_id = ? AND status = 'aktif' AND ? BETWEEN nilai_min AND nilai_max ORDER BY nilai_min DESC LIMIT 1`, [policyId, score]);
    return rows.length ? String(rows[0].predicateName || '') : (score >= 90 ? 'Baik Sekali' : score >= 76 ? 'Baik' : 'Perlu Pembinaan');
};
const getPolicyWeights = async (policyId) => {
    const defaults = { iku: 10, iki: 50, penugasan: 15, aktivitas: 15, perilaku: 10 };
    if (!policyId)
        return defaults;
    const [rows] = await database_1.pool.query(`SELECT komponen AS component, MAX(bobot) AS weight FROM kinerja_kebijakan_formula WHERE kebijakan_id = ? AND status = 'aktif' GROUP BY komponen`, [policyId]);
    for (const row of rows) {
        defaults[String(row.component)] = Number(row.weight || 0);
    }
    return defaults;
};
const applyRounding = (value, roundingType) => {
    if (roundingType === 'ke_atas')
        return Math.ceil(value * 100) / 100;
    if (roundingType === 'ke_bawah')
        return Math.floor(value * 100) / 100;
    return Math.round(value * 100) / 100;
};
const resolveComponentScore = async (policyId, component, rawScore) => {
    if (!policyId)
        return rawScore;
    const [rows] = await database_1.pool.query(`SELECT batas_min AS minScore, batas_max AS maxScore, pembulatan AS roundingType, penalti AS penalty, bonus
     FROM kinerja_kebijakan_formula
     WHERE kebijakan_id = ? AND komponen = ? AND status = 'aktif'
     ORDER BY id DESC LIMIT 1`, [policyId, component]);
    if (!rows.length)
        return rawScore;
    let value = rawScore;
    const row = rows[0];
    if (row.minScore != null && value < Number(row.minScore))
        value = Number(row.minScore);
    if (row.maxScore != null && value > Number(row.maxScore))
        value = Number(row.maxScore);
    if (row.penalty != null)
        value -= Number(row.penalty);
    if (row.bonus != null)
        value += Number(row.bonus);
    if (value < 0)
        value = 0;
    return applyRounding(value, String(row.roundingType || 'normal'));
};
exports.getKinerjaPolicies = (0, http_1.asyncHandler)(async (req, res) => {
    ...
    const where = [];
    if (periodId) {
        where.push('kp.periode_id = ?');
        params.push(periodId);
    }
    if (status) {
        where.push('kp.status = ?');
        params.push(status);
    }
    const [rows] = await database_1.pool.query(`SELECT kp.id, kp.periode_id AS periodeId, COALESCE(p.nama_periode, '-') AS periodeName,
     ...
     ORDER BY kp.tanggal_berlaku DESC, kp.id DESC`, params);
    return (0, http_1.sendSuccess)(res, rows.map(buildPolicyRecord));
});
exports.createKinerjaPolicy = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureEvaluationSchema();
    const payload = normalizePolicyPayload(req.body || {});
    if (payload.periodeId)
        await ensurePeriodExists(payload.periodeId);
    const [result] = await database_1.pool.query(`INSERT INTO kinerja_kebijakan_penilaian (periode_id, nama_kebijakan, versi, tanggal_berlaku, status, catatan)
     VALUES (?, ?, ?, ?, ?, ?)`, [payload.periodeId, payload.policyName, payload.version, payload.effectiveDate, payload.status, payload.note || null]);
    return (0, http_1.sendSuccess)(res, { id: result.insertId }, 'Kebijakan penilaian berhasil ditambahkan', 201);
});
exports.updateKinerjaPolicy = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureEvaluationSchema();
    const id = (0, validation_1.readPositiveId)(req.params.id, 'Kebijakan penilaian');
    await ensurePolicyExists(id);
    const payload = normalizePolicyPayload(req.body || {});
    if (payload.periodeId)
        await ensurePeriodExists(payload.periodeId);
    await database_1.pool.query(`UPDATE kinerja_kebijakan_penilaian SET periode_id = ?, nama_kebijakan = ?, versi = ?, tanggal_berlaku = ?, status = ?, catatan = ? WHERE id = ?`, [payload.periodeId, payload.policyName, payload.version, payload.effectiveDate, payload.status, payload.note || null, id]);
    return (0, http_1.sendSuccess)(res, null, 'Kebijakan penilaian berhasil diperbarui');
});
exports.activateKinerjaPolicy = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureEvaluationSchema();
    const id = (0, validation_1.readPositiveId)(req.params.id, 'Kebijakan penilaian');
    await ensurePolicyExists(id);
    const [[policy]] = await database_1.pool.query(`SELECT periode_id AS periodeId FROM kinerja_kebijakan_penilaian WHERE id = ? LIMIT 1`, [id]);
    if (policy?.periodeId) {
        await database_1.pool.query(`UPDATE kinerja_kebijakan_penilaian SET status = 'nonaktif' WHERE periode_id = ?`, [policy.periodeId]);
    }
    else {
        await database_1.pool.query(`UPDATE kinerja_kebijakan_penilaian SET status = 'nonaktif' WHERE periode_id IS NULL`);
    }
    await database_1.pool.query(`UPDATE kinerja_kebijakan_penilaian SET status = 'aktif' WHERE id = ?`, [id]);
    return (0, http_1.sendSuccess)(res, null, 'Kebijakan penilaian berhasil diaktifkan');
});
exports.getKinerjaPolicyFormulas = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureEvaluationSchema();
    const policyId = readOptionalPositiveId(req.query.policyId, 'Kebijakan penilaian');
    const params = [];
    const where = [];
    if (policyId) {
        where.push('f.kebijakan_id = ?');
        params.push(policyId);
    }
    const [rows] = await database_1.pool.query(`SELECT f.id, f.kebijakan_id AS policyId, kp.nama_kebijakan AS policyName, f.komponen AS component,
     ...
     ${where.length ? `WHERE ${where.join(' AND ')}` : ''}
     ORDER BY kp.nama_kebijakan ASC, FIELD(f.komponen, 'iku','iki','penugasan','aktivitas','perilaku'), f.id DESC`, params);
    return (0, http_1.sendSuccess)(res, rows.map(buildFormulaRecord));
});
exports.createKinerjaPolicyFormula = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureEvaluationSchema();
    const payload = normalizeFormulaPayload(req.body || {});
    await ensurePolicyExists(payload.policyId);
    const [result] = await database_1.pool.query(`INSERT INTO kinerja_kebijakan_formula (kebijakan_id, komponen, jenis_jabatan, bobot, formula, batas_min, batas_max, pembulatan, penalti, bonus, status)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [payload.policyId, payload.component, payload.jobType, payload.weight, payload.formula || null, payload.minScore, payload.maxScore, payload.roundingType, payload.penalty, payload.bonus, payload.status]);
    return (0, http_1.sendSuccess)(res, { id: result.insertId }, 'Formula penilaian berhasil ditambahkan', 201);
});
exports.updateKinerjaPolicyFormula = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureEvaluationSchema();
    const id = (0, validation_1.readPositiveId)(req.params.id, 'Formula penilaian');
    const payload = normalizeFormulaPayload(req.body || {});
    await ensurePolicyExists(payload.policyId);
    await database_1.pool.query(`UPDATE kinerja_kebijakan_formula SET kebijakan_id = ?, komponen = ?, jenis_jabatan = ?, bobot = ?, formula = ?, batas_min = ?, batas_max = ?, pembulatan = ?, penalti = ?, bonus = ?, status = ? WHERE id = ?`, [payload.policyId, payload.component, payload.jobType, payload.weight, payload.formula || null, payload.minScore, payload.maxScore, payload.roundingType, payload.penalty, payload.bonus, payload.status, id]);
    return (0, http_1.sendSuccess)(res, null, 'Formula penilaian berhasil diperbarui');
});
exports.getKinerjaPredicates = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureEvaluationSchema();
    const policyId = readOptionalPositiveId(req.query.policyId, 'Kebijakan penilaian');
    const params = [];
    const where = [];
    if (policyId) {
        where.push('p.kebijakan_id = ?');
        params.push(policyId);
    }
    const [rows] = await database_1.pool.query(`SELECT p.id, p.kebijakan_id AS policyId, kp.nama_kebijakan AS policyName, p.nilai_min AS minScore,
     ...
     ${where.length ? `WHERE ${where.join(' AND ')}` : ''}
     ORDER BY kp.nama_kebijakan ASC, p.nilai_min DESC, p.id DESC`, params);
    return (0, http_1.sendSuccess)(res, rows.map(buildPredicateRecord));
});
exports.createKinerjaPredicate = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureEvaluationSchema();
    const payload = normalizePredicatePayload(req.body || {});
    await ensurePolicyExists(payload.policyId);
    const [result] = await database_1.pool.query(`INSERT INTO kinerja_kebijakan_predikat (kebijakan_id, nilai_min, nilai_max, nama_predikat, rekomendasi, status)
     VALUES (?, ?, ?, ?, ?, ?)`, [payload.policyId, payload.minScore, payload.maxScore, payload.predicateName, payload.recommendation || null, payload.status]);
    return (0, http_1.sendSuccess)(res, { id: result.insertId }, 'Predikat kinerja berhasil ditambahkan', 201);
});
exports.updateKinerjaPredicate = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureEvaluationSchema();
    const id = (0, validation_1.readPositiveId)(req.params.id, 'Predikat kinerja');
    const payload = normalizePredicatePayload(req.body || {});
    await ensurePolicyExists(payload.policyId);
    await database_1.pool.query(`UPDATE kinerja_kebijakan_predikat SET kebijakan_id = ?, nilai_min = ?, nilai_max = ?, nama_predikat = ?, rekomendasi = ?, status = ? WHERE id = ?`, [payload.policyId, payload.minScore, payload.maxScore, payload.predicateName, payload.recommendation || null, payload.status, id]);
    return (0, http_1.sendSuccess)(res, null, 'Predikat kinerja berhasil diperbarui');
});
exports.getKinerjaMidyearEvaluations = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureEvaluationSchema();
    const periodId = readOptionalPositiveId(req.query.periodeId, 'Periode kinerja');
    const params = [];
    const where = [];
    if (periodId) {
        where.push('e.periode_id = ?');
        params.push(periodId);
    }
    const [rows] = await database_1.pool.query(`SELECT e.id, e.periode_id AS periodeId, p.nama_periode AS periodeName,
     ...
     ${where.length ? `WHERE ${where.join(' AND ')}` : ''}
     ORDER BY e.tanggal_evaluasi DESC, emp.nama_lengkap ASC`, params);
    return (0, http_1.sendSuccess)(res, rows.map(buildMidyearRecord));
});
exports.createKinerjaMidyearEvaluation = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureEvaluationSchema();
    ...
    if (payload.policyId)
        await ensurePolicyExists(payload.policyId);
    const [result] = await database_1.pool.query(`INSERT INTO kinerja_evaluasi_tengah_tahun
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
     ON DUPLICATE KEY UPDATE penilai_id = VALUES(penilai_id), kebijakan_id = VALUES(kebijakan_id), ringkasan_capaian = VALUES(ringkasan_capaian), hambatan = VALUES(hambatan), rencana_perbaikan = VALUES(rencana_perbaikan), nilai_sementara = VALUES(nilai_sementara), catatan_penilai = VALUES(catatan_penilai), tanggal_evaluasi = VALUES(tanggal_evaluasi), status = VALUES(status)`, [payload.periodeId, payload.employeeId, payload.evaluatorId, payload.policyId, payload.achievementSummary || null, payload.obstacles || null, payload.improvementPlan || null, payload.temporaryScore, payload.evaluatorNote || null, payload.evaluationDate, payload.status]);
    return (0, http_1.sendSuccess)(res, { id: result.insertId || 0 }, 'Evaluasi tengah tahun berhasil disimpan', 201);
});
exports.updateKinerjaMidyearEvaluation = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureEvaluationSchema();
    const id = (0, validation_1.readPositiveId)(req.params.id, 'Evaluasi tengah tahun');
    ...
    if (payload.policyId)
        await ensurePolicyExists(payload.policyId);
    await database_1.pool.query(`UPDATE kinerja_evaluasi_tengah_tahun SET periode_id = ?, pegawai_id = ?, penilai_id = ?, kebijakan_id = ?, ringkasan_capaian = ?, hambatan = ?, rencana_perbaikan = ?, nilai_sementara = ?, catatan_penilai = ?, tanggal_evaluasi = ?, status = ? WHERE id = ?`, [payload.periodeId, payload.employeeId, payload.evaluatorId, payload.policyId, payload.achievementSummary || null, payload.obstacles || null, payload.improvementPlan || null, payload.temporaryScore, payload.evaluatorNote || null, payload.evaluationDate, payload.status, id]);
    return (0, http_1.sendSuccess)(res, null, 'Evaluasi tengah tahun berhasil diperbarui');
});
exports.finalizeKinerjaMidyearEvaluation = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureEvaluationSchema();
    const id = (0, validation_1.readPositiveId)(req.params.id, 'Evaluasi tengah tahun');
    await ensureMidyearEvaluationExists(id);
    await database_1.pool.query(`UPDATE kinerja_evaluasi_tengah_tahun SET status = 'final' WHERE id = ?`, [id]);
    return (0, http_1.sendSuccess)(res, null, 'Evaluasi tengah tahun berhasil difinalkan');
});
exports.getKinerjaFinalEvaluations = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureEvaluationSchema();
    const periodId = readOptionalPositiveId(req.query.periodeId, 'Periode kinerja');
    const params = [];
    const where = [];
    ...
    const [rows] = await database_1.pool.query(`SELECT e.id, e.periode_id AS periodeId, p.nama_periode AS periodeName,
     ...
     ${where.length ? `WHERE ${where.join(' AND ')}` : ''}
     ORDER BY e.tanggal_evaluasi DESC, emp.nama_lengkap ASC`, params);
    return (0, http_1.sendSuccess)(res, rows.map(buildFinalRecord));
});
exports.generateKinerjaFinalEvaluations = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureEvaluationSchema();
    const periodId = (0, validation_1.readPositiveId)(req.body?.periodeId, 'Periode kinerja');
    ...
    const evaluationDate = (0, validation_1.readDateString)(req.body?.evaluationDate || new Date().toISOString().slice(0, 10), 'Tanggal evaluasi');
    await ensurePeriodExists(periodId);
    await ensureEmployeeExists(evaluatorId);
    if (policyId)
        await ensurePolicyExists(policyId);
    ...
    const [rows] = await database_1.pool.query(`SELECT e.id AS employeeId,
     ...
     ORDER BY e.nama_lengkap ASC`, [periodId, periodId, periodId, periodId]);
    let processed = 0;
    for (const row of rows) {
        ...
        const finalScore = applyRounding(((ikuScore * weights.iku) + (ikiScore * weights.iki) + (assignmentScore * weights.penugasan) + (activityScore * weights.aktivitas) + (behaviorScore * weights.perilaku)) / totalWeight, 'normal');
        const predicate = await resolvePredicateByPolicy(policyId, finalScore);
        await database_1.pool.query(`INSERT INTO kinerja_evaluasi_akhir_tahun
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'generated')
       ON DUPLICATE KEY UPDATE penilai_id = VALUES(penilai_id), kebijakan_id = VALUES(kebijakan_id), nilai_iku = VALUES(nilai_iku), nilai_iki = VALUES(nilai_iki), nilai_penugasan = VALUES(nilai_penugasan), nilai_aktivitas = VALUES(nilai_aktivitas), nilai_perilaku = VALUES(nilai_perilaku), nilai_akhir = VALUES(nilai_akhir), predikat = VALUES(predikat), tanggal_evaluasi = VALUES(tanggal_evaluasi), status = 'generated'`, [periodId, Number(row.employeeId), evaluatorId, policyId, ikuScore, ikiScore, assignmentScore, activityScore, behaviorScore, finalScore, predicate, 'Dihasilkan otomatis dari data Fase 1 sampai 4', evaluationDate]);
        processed += 1;
    }
    return (0, http_1.sendSuccess)(res, { processed }, 'Evaluasi akhir tahun berhasil dihasilkan');
});
exports.updateKinerjaFinalEvaluation = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureEvaluationSchema();
    const id = (0, validation_1.readPositiveId)(req.params.id, 'Evaluasi akhir tahun');
    ...
    await database_1.pool.query(`UPDATE kinerja_evaluasi_akhir_tahun SET periode_id = ?, pegawai_id = ?, penilai_id = ?, kebijakan_id = ?, nilai_iku = ?, nilai_iki = ?, nilai_penugasan = ?, nilai_aktivitas = ?, nilai_perilaku = ?, nilai_akhir = ?, predikat = ?, catatan_penilai = ?, tanggal_evaluasi = ?, status = ? WHERE id = ?`, [payload.periodeId, payload.employeeId, payload.evaluatorId, payload.policyId, payload.ikuScore, payload.ikiScore, payload.assignmentScore, payload.activityScore, payload.behaviorScore, payload.finalScore, predicate, payload.evaluatorNote || null, payload.evaluationDate, payload.status, id]);
    return (0, http_1.sendSuccess)(res, null, 'Evaluasi akhir tahun berhasil diperbarui');
});
exports.finalizeKinerjaFinalEvaluation = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureEvaluationSchema();
    const id = (0, validation_1.readPositiveId)(req.params.id, 'Evaluasi akhir tahun');
    await ensureFinalEvaluationExists(id);
    await database_1.pool.query(`UPDATE kinerja_evaluasi_akhir_tahun SET status = 'finalized' WHERE id = ?`, [id]);
    return (0, http_1.sendSuccess)(res, null, 'Evaluasi akhir tahun berhasil difinalkan');
});
exports.getKinerjaCalibrations = (0, http_1.asyncHandler)(async (_req, res) => {
    await ensureEvaluationSchema();
    const [sessions] = await database_1.pool.query(`SELECT c.id, c.periode_id AS periodeId, p.nama_periode AS periodeName, c.nama_sesi AS sessionName,
    ...
    const [items] = await database_1.pool.query(`SELECT i.id, i.kalibrasi_id AS calibrationId, i.evaluasi_akhir_id AS evaluationId, i.pegawai_id AS employeeId,
    ...
    return (0, http_1.sendSuccess)(res, sessions.map((session) => buildCalibrationRecord(session, grouped.get(Number(session.id)) || [])));
});
exports.createKinerjaCalibration = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureEvaluationSchema();
    ...
    const [result] = await database_1.pool.query(`INSERT INTO kinerja_kalibrasi (periode_id, nama_sesi, tanggal_kalibrasi, ketua_sesi_id, catatan, status)
     VALUES (?, ?, ?, ?, ?, ?)`, [payload.periodeId, payload.sessionName, payload.calibrationDate, payload.chairpersonId, payload.note || null, payload.status]);
    return (0, http_1.sendSuccess)(res, { id: result.insertId }, 'Sesi kalibrasi berhasil ditambahkan', 201);
});
exports.updateKinerjaCalibration = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureEvaluationSchema();
    const id = (0, validation_1.readPositiveId)(req.params.id, 'Sesi kalibrasi');
    await ensureCalibrationExists(id);
    const payload = normalizeCalibrationPayload(req.body || {}, req.user?.employeeId || null);
    await ensurePeriodExists(payload.periodeId);
    await ensureEmployeeExists(payload.chairpersonId);
    await database_1.pool.query(`UPDATE kinerja_kalibrasi SET periode_id = ?, nama_sesi = ?, tanggal_kalibrasi = ?, ketua_sesi_id = ?, catatan = ?, status = ? WHERE id = ?`, [payload.periodeId, payload.sessionName, payload.calibrationDate, payload.chairpersonId, payload.note || null, payload.status, id]);
    return (0, http_1.sendSuccess)(res, null, 'Sesi kalibrasi berhasil diperbarui');
});
exports.createKinerjaCalibrationItem = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureEvaluationSchema();
    const calibrationId = (0, validation_1.readPositiveId)(req.params.id, 'Sesi kalibrasi');
    ...
    if (payload.reviewerId)
        await ensureEmployeeExists(payload.reviewerId);
    const [result] = await database_1.pool.query(`INSERT INTO kinerja_kalibrasi_item (kalibrasi_id, evaluasi_akhir_id, pegawai_id, nilai_sebelum, nilai_sesudah, alasan_perubahan, reviewer_id)
     VALUES (?, ?, ?, ?, ?, ?, ?)
     ON DUPLICATE KEY UPDATE pegawai_id = VALUES(pegawai_id), nilai_sebelum = VALUES(nilai_sebelum), nilai_sesudah = VALUES(nilai_sesudah), alasan_perubahan = VALUES(alasan_perubahan), reviewer_id = VALUES(reviewer_id)`, [calibrationId, payload.evaluationId, payload.employeeId, payload.previousScore, payload.adjustedScore, payload.reason || null, payload.reviewerId]);
    return (0, http_1.sendSuccess)(res, { id: result.insertId || 0 }, 'Item kalibrasi berhasil disimpan', 201);
});
exports.updateKinerjaCalibrationItem = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureEvaluationSchema();
    const calibrationId = (0, validation_1.readPositiveId)(req.params.id, 'Sesi kalibrasi');
    const itemId = (0, validation_1.readPositiveId)(req.params.itemId, 'Item kalibrasi');
    await ensureCalibrationExists(calibrationId);
    ...
    if (payload.reviewerId)
        await ensureEmployeeExists(payload.reviewerId);
    await database_1.pool.query(`UPDATE kinerja_kalibrasi_item SET evaluasi_akhir_id = ?, pegawai_id = ?, nilai_sebelum = ?, nilai_sesudah = ?, alasan_perubahan = ?, reviewer_id = ? WHERE id = ? AND kalibrasi_id = ?`, [payload.evaluationId, payload.employeeId, payload.previousScore, payload.adjustedScore, payload.reason || null, payload.reviewerId, itemId, calibrationId]);
    return (0, http_1.sendSuccess)(res, null, 'Item kalibrasi berhasil diperbarui');
});
exports.closeKinerjaCalibration = (0, http_1.asyncHandler)(async (req, res) => {
    await ensureEvaluationSchema();
    const calibrationId = (0, validation_1.readPositiveId)(req.params.id, 'Sesi kalibrasi');
    await ensureCalibrationExists(calibrationId);
    const [rows] = await database_1.pool.query(`SELECT i.evaluasi_akhir_id AS evaluationId, i.nilai_sesudah AS adjustedScore, e.kebijakan_id AS policyId
     FROM kinerja_kalibrasi_item i
     INNER JOIN kinerja_evaluasi_akhir_tahun e ON e.id = i.evaluasi_akhir_id
     WHERE i.kalibrasi_id = ?`, [calibrationId]);
    for (const row of rows) {
        ...
        await database_1.pool.query(`UPDATE kinerja_evaluasi_akhir_tahun SET nilai_akhir = COALESCE(?, nilai_akhir), predikat = COALESCE(?, predikat), status = 'calibrated' WHERE id = ?`, [adjustedScore, predicate, Number(row.evaluationId)]);
    }
    await database_1.pool.query(`UPDATE kinerja_kalibrasi SET status = 'closed' WHERE id = ?`, [calibrationId]);
    return (0, http_1.sendSuccess)(res, null, 'Sesi kalibrasi berhasil ditutup');
});
