...
exports.saveAttendanceAssessment = exports.getAttendanceAssessments = void 0;
...
const getQuarterNumber = (month) => Math.ceil(month / 3);
const calculatePenaltyIndex = (payload) => {
    return Number(((payload.tl1Count || 0) * ATTENDANCE_INDEX.tl1 +
        ...
        (payload.psw4Count || 0) * ATTENDANCE_INDEX.psw4).toFixed(2));
};
const calculateMonthlyScore = (attendanceDays, penaltyIndex) => {
    const ratioPenalty = (penaltyIndex / Math.max(attendanceDays, 1)) * 100;
    return Number(Math.max(0, 100 - ratioPenalty).toFixed(2));
};
exports.getAttendanceAssessments = (0, http_1.asyncHandler)(async (req, res) => {
...
exports.saveAttendanceAssessment = (0, http_1.asyncHandler)(async (req, res) => {
    ...
    const [employeeRows] = await database_1.pool.query(`SELECT id FROM pegawai WHERE id = ? LIMIT 1`, [payload.employeeId]);
    if (!employeeRows || !employeeRows.length)
        (0, http_1.fail)("Pegawai tidak ditemukan", 404);
    const [existing] = await database_1.pool.query(`SELECT id FROM penilaian_kehadiran WHERE pegawai_id=? AND tahun_penilaian=? AND bulan_penilaian=? LIMIT 1`, [payload.employeeId, payload.assessmentYear, payload.assessmentMonth]);
    let attendanceId = Number(existing[0]?.id || 0);
    if (attendanceId) {
        await database_1.pool.query(`UPDATE penilaian_kehadiran SET hari_kehadiran=?, jumlah_tl1=?, jumlah_tl2=?, jumlah_tl3=?, jumlah_tl4=?, jumlah_psw1=?, jumlah_psw2=?, jumlah_psw3=?, jumlah_psw4=?, total_indeks_pengurang=?, skor_bulanan=?, triwulan=? WHERE id=?`, [
        ...
        const [result] = await database_1.pool.query(`INSERT INTO penilaian_kehadiran (pegawai_id, tahun_penilaian, bulan_penilaian, hari_kehadiran, jumlah_tl1, jumlah_tl2, jumlah_tl3, jumlah_tl4, jumlah_psw1, jumlah_psw2, jumlah_psw3, jumlah_psw4, total_indeks_pengurang, skor_bulanan, triwulan) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
            payload.employeeId,
    ...
    return (0, http_1.sendSuccess)(res, { id: attendanceId }, "Rekap kehadiran berhasil disimpan", 201);
});
