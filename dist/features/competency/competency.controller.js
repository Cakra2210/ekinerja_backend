...
exports.deleteCompetencyDevelopmentActivity = exports.updateCompetencyDevelopmentActivity = exports.createCompetencyDevelopmentActivity = exports.getCompetencyDevelopmentRecap = void 0;
...
const ACTIVITY_ROLES = Object.keys(ROLE_INDEX_MAP);
const UPLOAD_ROOT = path_1.default.resolve(process.cwd(), "uploads");
const toQuarter = (dateValue) => {
...
const readOptionalQueryNumber = (value) => {
...
const toPublicUploadPath = (storedPath) => {
    if (!storedPath) {
        return null;
    }
    return storedPath.replace(/\\/g, "/");
};
const removeUploadFile = (storedPath) => {
    if (!storedPath) {
        return;
    }
    const normalizedPath = storedPath.replace(/\\/g, "/");
    const absolutePath = path_1.default.isAbsolute(normalizedPath)
        ? path_1.default.resolve(normalizedPath)
        : path_1.default.resolve(UPLOAD_ROOT, normalizedPath.replace(/^\/+/, "").replace(/^uploads\//, ""));
...
const normalizePayload = (body) => {
    const payload = {
        ...
        activityName: (0, validation_1.ensureRequired)((0, validation_1.readTrimmedString)(body.activityName), "Nama kegiatan wajib diisi"),
        activityType: (0, validation_1.ensureRequired)((0, validation_1.readTrimmedString)(body.activityType), "Jenis kegiatan wajib diisi"),
        startDate: (0, validation_1.readDateString)(body.startDate, "Tanggal mulai kegiatan"),
        endDate: (0, validation_1.readDateString)(body.endDate, "Tanggal selesai kegiatan"),
        activityRole: (0, validation_1.ensureOneOf)((0, validation_1.readTrimmedString)(body.activityRole || "peserta").toLowerCase(), ACTIVITY_ROLES, "Peran kegiatan"),
        learningHours: (0, validation_1.readPositiveNumber)(body.learningHours, "Jam pembelajaran"),
        note: (0, validation_1.readTrimmedString)(body.note)
    };
    if (new Date(payload.endDate).getTime() < new Date(payload.startDate).getTime()) {
        (0, http_1.fail)("Tanggal selesai kegiatan tidak boleh lebih awal dari tanggal mulai kegiatan", 400);
    }
    return payload;
};
...
const fetchActivityById = async (activityId) => {
    const [rows] = await database_1.pool.query(`SELECT cd.id,
...
const ensureEmployeeExists = async (employeeId) => {
    ...
    if (!employeeRows.length) {
        (0, http_1.fail)("Pegawai tidak ditemukan", 400);
    }
};
exports.getCompetencyDevelopmentRecap = (0, http_1.asyncHandler)(async (req, res) => {
    try {
        ...
        const activityWhereClause = activityConditions.length
            ...
            : "";
        const [activities] = await database_1.pool.query(`SELECT cd.id,
              cd.pegawai_id AS employeeId,
              e.nama_lengkap AS employeeName,
              e.nip,
              COALESCE(p.nama, e.nama_jabatan) AS position,
       ...
       LEFT JOIN jabatan p ON p.id = e.jabatan_id
       ${activityWhereClause}
       ORDER BY cd.tanggal_selesai DESC, cd.dibuat_pada DESC`, activityParams);
        const recapConditions = [];
        const recapParams = [];
        if (hasYearFilter) {
        ...
        const [recap] = await database_1.pool.query(`SELECT e.id AS employeeId,
              e.nama_lengkap AS employeeName,
              e.nip,
              COALESCE(p.nama, e.nama_jabatan) AS position,
       ...
       ${employeeOuterWhere}
       ORDER BY quarterScore DESC, equivalentHours DESC, employeeName ASC`, [QUARTER_TARGET_HOURS, QUARTER_TARGET_HOURS, ...recapParams, ...employeeParams]);
        ...
        if (error?.code === "ER_NO_SUCH_TABLE") {
            (0, http_1.fail)("Tabel pengembangan kompetensi belum tersedia. Pastikan database diimpor dari backend/db/kinerja_pegawai_bps.sql.", 500);
        }
        throw error;
    }
});
exports.createCompetencyDevelopmentActivity = (0, http_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        if (!quarter) {
            (0, http_1.fail)("Tanggal selesai kegiatan tidak valid", 400);
        }
        ...
        const [result] = await database_1.pool.query(`INSERT INTO kegiatan_pengembangan_kompetensi
       (
         pegawai_id,
         nama_kegiatan,
         jenis_kegiatan,
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
            ...
            invitationFile ? toPublicUploadPath(path_1.default.relative(process.cwd(), invitationFile.path)) : null,
            certificateFile?.originalname || null,
            certificateFile ? toPublicUploadPath(path_1.default.relative(process.cwd(), certificateFile.path)) : null,
            payload.note || null
        ]);
        const created = await fetchActivityById(Number(result.insertId));
        return (0, http_1.sendSuccess)(res, created, "Aktivitas pengembangan kompetensi berhasil disimpan", 201);
    }
    catch (error) {
...
exports.updateCompetencyDevelopmentActivity = (0, http_1.asyncHandler)(async (req, res) => {
    const activityId = (0, validation_1.readPositiveId)(req.params.id, "ID aktivitas");
    const uploadedFiles = (req.files || {});
    const invitationFile = uploadedFiles.invitationFile?.[0];
    const certificateFile = uploadedFiles.certificateFile?.[0];
    try {
       ...
       LIMIT 1`, [activityId]);
        ...
        const nextInvitationPath = invitationFile
            ? toPublicUploadPath(path_1.default.relative(process.cwd(), invitationFile.path))
            : existing.invitationFilePath;
        const nextCertificatePath = certificateFile
            ? toPublicUploadPath(path_1.default.relative(process.cwd(), certificateFile.path))
            : existing.certificateFilePath;
        await database_1.pool.query(`UPDATE kegiatan_pengembangan_kompetensi
       SET pegawai_id = ?,
           nama_kegiatan = ?,
           jenis_kegiatan = ?,
           tanggal_kegiatan = ?,
       ...
       WHERE id = ?`, [
        ...
        return (0, http_1.sendSuccess)(res, updated, "Aktivitas pengembangan kompetensi berhasil diperbarui");
    }
    catch (error) {
        removeUploadFile(invitationFile?.path);
        removeUploadFile(certificateFile?.path);
...
exports.deleteCompetencyDevelopmentActivity = (0, http_1.asyncHandler)(async (req, res) => {
    const activityId = (0, validation_1.readPositiveId)(req.params.id, "ID aktivitas");
    const [rows] = await database_1.pool.query(`SELECT path_file_undangan AS invitationFilePath,
            path_file_sertifikat AS certificateFilePath
     FROM kegiatan_pengembangan_kompetensi
     WHERE id = ?
     LIMIT 1`, [activityId]);
    ...
    await database_1.pool.query(`DELETE FROM kegiatan_pengembangan_kompetensi WHERE id = ?`, [activityId]);
    removeUploadFile(rows[0].invitationFilePath);
    removeUploadFile(rows[0].certificateFilePath);
    return (0, http_1.sendSuccess)(res, null, "Aktivitas pengembangan kompetensi berhasil dihapus");
});
