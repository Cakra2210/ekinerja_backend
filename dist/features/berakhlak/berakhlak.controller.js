...
exports.getBerakhlakDashboard = exports.updateBerakhlakEvaluation = exports.createBerakhlakEvaluation = exports.getBerakhlakEvaluations = void 0;
...
const readOptionalQueryNumber = (value, fallback = 0) => {
...
const readScore = (value) => clampScore(readOptionalQueryNumber(value, 50));
const normalizePayload = (body) => ({
    employeeId: (0, validation_1.readPositiveId)(body.employeeId, "Pegawai yang dinilai"),
    evaluatorEmployeeId: (0, validation_1.readPositiveId)(body.evaluatorEmployeeId, "Pegawai penilai"),
    evaluationYear: (0, validation_1.readIntegerInRange)(body.evaluationYear, 2000, 2100, "Tahun penilaian"),
    evaluationMonth: (0, validation_1.readIntegerInRange)(body.evaluationMonth, 1, 12, "Bulan penilaian"),
...
const calculateDimensions = (payload) => {
    const pelayananAvg = calculateAverage(payload.pelayananResponsif, payload.pelayananRamah, payload.pelayananSolutif);
    const akuntabelAvg = calculateAverage(payload.akuntabelProsedur, payload.akuntabelTransparansi, payload.akuntabelTanggungJawab);
    const kompetenAvg = calculateAverage(payload.kompetenPenguasaan, payload.kompetenPenyelesaian, payload.kompetenPengembangan);
    const harmonisAvg = calculateAverage(payload.harmonisTim, payload.harmonisRelasi, payload.harmonisLingkungan);
    const loyalAvg = calculateAverage(payload.loyalKomitmen, payload.loyalAturan, payload.loyalDedikasi);
    const adaptifAvg = calculateAverage(payload.adaptifPerubahan, payload.adaptifFleksibilitas, payload.adaptifBelajar);
    const kolaboratifAvg = calculateAverage(payload.kolaboratifKerjaSama, payload.kolaboratifDiskusi, payload.kolaboratifKoordinasi);
    const finalScore = Number(((pelayananAvg +
        akuntabelAvg +
        kompetenAvg +
        harmonisAvg +
        loyalAvg +
...
const formatEvaluationRows = (rows) => rows.map((row) => ({
    id: Number(row.id),
...
const getSingleEvaluationById = async (id) => {
    const [rows] = await database_1.pool.query(`${BASE_SELECT}
     WHERE ev.id = ?
     LIMIT 1`, [id]);
...
const ensureValidPayload = (payload) => {
    if (payload.employeeId === payload.evaluatorEmployeeId) {
        (0, http_1.fail)("Pegawai tidak dapat menilai dirinya sendiri", 400);
    }
};
const getExistingEvaluation = async (id) => {
    const [currentRows] = await database_1.pool.query(`SELECT id, pegawai_id AS employeeId, penilai_pegawai_id AS evaluatorEmployeeId
...
const resolvePayloadByRole = (req, payload) => {
    if (req.user?.role === "pegawai") {
        if (!req.user.employeeId) {
            (0, http_1.fail)("Akun pegawai belum terhubung dengan data pegawai", 403);
        }
        return {
            ...payload,
...
const getDuplicateEvaluation = async (payload, excludeId) => {
    const conditions = [
        "penilai_pegawai_id = ?",
        "pegawai_id = ?",
        "tahun_evaluasi = ?",
...
exports.getBerakhlakEvaluations = (0, http_1.asyncHandler)(async (req, res) => {
    try {
        const whereClauses = [];
        const params = [];
        if (req.user?.role === "pegawai") {
        ...
        const [rows] = await database_1.pool.query(`${BASE_SELECT}${whereSql}
       ORDER BY ev.diperbarui_pada DESC, ev.dibuat_pada DESC, ev.id DESC`, params);
        return (0, http_1.sendSuccess)(res, formatEvaluationRows(rows));
    }
    catch (error) {
        if (error?.code === "ER_NO_SUCH_TABLE") {
            (0, http_1.fail)("Tabel penilaian 360 BerAKHLAK belum tersedia. Pastikan struktur database sudah diimpor dari sql/kinerja_pegawai_bps.sql.", 500);
        }
        throw error;
    }
});
exports.createBerakhlakEvaluation = (0, http_1.asyncHandler)(async (req, res) => {
    try {
        const payload = resolvePayloadByRole(req, normalizePayload(req.body));
        ensureValidPayload(payload);
        const duplicate = await getDuplicateEvaluation(payload);
        if (duplicate) {
            (0, http_1.fail)("Pegawai ini sudah Anda nilai pada bulan dan tahun yang sama. Gunakan tombol edit untuk memperbarui nilai periode tersebut.", 409);
        }
        ...
        return (0, http_1.sendSuccess)(res, created, "Penilaian 360 BerAKHLAK berhasil disimpan", 201);
    }
    catch (error) {
...
exports.updateBerakhlakEvaluation = (0, http_1.asyncHandler)(async (req, res) => {
    try {
        const id = (0, validation_1.readPositiveId)(req.params.id, "ID penilaian");
        ...
        if (req.user?.role === "pegawai" && existingEvaluation.evaluatorEmployeeId !== req.user.employeeId) {
            (0, http_1.fail)("Anda hanya dapat mengubah penilaian yang Anda buat sendiri", 403);
        }
        ensureValidPayload(payload);
        const duplicate = await getDuplicateEvaluation(payload, id);
        if (duplicate) {
            (0, http_1.fail)("Pegawai ini sudah Anda nilai pada bulan dan tahun yang sama. Hanya satu data diizinkan untuk setiap periode bulan.", 409);
        }
        const dimensions = calculateDimensions(payload);
       ...
       WHERE id = ?`, [
        ...
        return (0, http_1.sendSuccess)(res, updated, "Penilaian 360 BerAKHLAK berhasil diperbarui");
    }
    catch (error) {
        if (error?.code === "ER_DUP_ENTRY") {
            (0, http_1.fail)("Pegawai ini sudah Anda nilai pada bulan dan tahun yang sama. Hanya satu data penilaian yang diizinkan untuk periode tersebut.", 409);
        }
        if (error?.code === "ER_NO_SUCH_TABLE") {
            (0, http_1.fail)("Tabel penilaian 360 BerAKHLAK belum tersedia. Pastikan struktur database sudah diimpor dari sql/kinerja_pegawai_bps.sql.", 500);
        }
...
exports.getBerakhlakDashboard = (0, http_1.asyncHandler)(async (req, res) => {
    try {
       ...
       ORDER BY ev.diperbarui_pada DESC, ev.dibuat_pada DESC, ev.id DESC`, params);
        ...
        const employeeScores = selectedEmployeeId > 0 && selectedMonth <= 0
            ? Array.from(formattedRows.reduce((map, item) => {
