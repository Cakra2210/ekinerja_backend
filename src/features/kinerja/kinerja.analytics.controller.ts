...
const readSelectedPeriod = async (rawPeriodId: unknown) => {
  ...
  if (rawPeriodId !== undefined && rawPeriodId !== null && rawPeriodId !== "") {
    ...
    if (Number.isFinite(periodId) && periodId > 0) {
      const [rows] = await pool.query<any[]>(
        `SELECT id, tahun, nama_periode AS namaPeriode, status FROM kinerja_periode WHERE id = ? LIMIT 1`,
        [periodId]
      );
      if (rows.length) {
        return {
  ...
  const [activeRows] = await pool.query<any[]>(
    `SELECT id, tahun, nama_periode AS namaPeriode, status
...
const periodFilter = (columnName: string, periodId: number | null) =>
  periodId ? ` WHERE ${columnName} = ${Number(periodId)} ` : "";

const withCondition = (baseFilter: string, condition: string) => {
  if (baseFilter.trim()) {
...
export const getKinerjaExecutiveAnalytics = asyncHandler(async (req, res) => {
  ...
  const [employeeRows] = await pool.query<any[]>(
    `SELECT COUNT(DISTINCT iki.pegawai_id) AS totalEmployees,
            COUNT(*) AS totalIki
     FROM kinerja_iki_pegawai iki
     ${periodFilter("iki.periode_id", period.periodId)}`
  );

  const [evaluationRows] = await pool.query<any[]>(
    `SELECT COUNT(*) AS totalFinalEvaluations,
  ...
  const [realizationRows] = await pool.query<any[]>(
    `SELECT COUNT(*) AS totalRealizations,
  ...
  const [bpsRows] = await pool.query<any[]>(
    `SELECT
  ...
  const [topTeams] = await pool.query<any[]>(
    `SELECT tk.id,
  ...
  const [topEmployees] = await pool.query<any[]>(
    `SELECT ev.pegawai_id AS employeeId,
...
export const getKinerjaTeamRankings = asyncHandler(async (req, res) => {
  const period = await readSelectedPeriod(req.query.periodId ?? req.query.periodeId);
...
export const getKinerjaScoreDistribution = asyncHandler(async (req, res) => {
  const period = await readSelectedPeriod(req.query.periodId ?? req.query.periodeId);

  const [rangeRows] = await pool.query<any[]>(
  ...
  const [predicateRows] = await pool.query<any[]>(
    `SELECT COALESCE(predikat, 'Belum ditetapkan') AS predicateName,
            COUNT(*) AS total,
  ...
  const [scoreSummaryRows] = await pool.query<any[]>(
...
export const getKinerjaWorkloadAnalytics = asyncHandler(async (req, res) => {
  const period = await readSelectedPeriod(req.query.periodId ?? req.query.periodeId);
...
export const getKinerjaContributionAnalytics = asyncHandler(async (req, res) => {
  const period = await readSelectedPeriod(req.query.periodId ?? req.query.periodeId);

  const [ikuRows] = await pool.query<any[]>(
    `SELECT iku.id,
