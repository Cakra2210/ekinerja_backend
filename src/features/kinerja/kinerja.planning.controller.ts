...
const planningStatuses = ["draft", "aktif", "arsip"] as const;
const ikiStatuses = ["draft", "diajukan", "disetujui", "revisi", "dikunci"] as const;
const targetPeriodTypes = ["bulanan", "triwulan", "semester"] as const;
const targetPeriodStatuses = ["draft", "aktif", "dikunci"] as const;
const dialogStatuses = ["draft", "diajukan", "disetujui"] as const;
...
const ensureRecordExists = async (tableName: string, id: number, label: string) => {
  const [rows] = await pool.query<any[]>(`SELECT id FROM ${tableName} WHERE id = ? LIMIT 1`, [id]);
  if (!rows.length) fail(`${label} tidak ditemukan`, 404);
};

const normalizeIkuPayload = (body: Record<string, unknown>) => ({
  ...
  status: ensureOneOf(readTrimmedString(body.status || "draft").toLowerCase(), planningStatuses, "Status")
});

const normalizeCascadingPayload = (body: Record<string, unknown>) => ({
...
const normalizeIkiPayload = (body: Record<string, unknown>) => ({
  periodeId: readPositiveId(body.periodeId, "Periode kinerja"),
  ...
  status: ensureOneOf(readTrimmedString(body.status || "draft").toLowerCase(), ["draft", "revisi"], "Status awal")
});

const normalizeTargetPeriodikPayload = (body: Record<string, unknown>) => {
  const startDate = readDateString(body.tanggalMulai, "Tanggal mulai");
  const endDate = readDateString(body.tanggalSelesai, "Tanggal selesai");
  ensureDateRange(startDate, endDate);
  return {
    ikiPegawaiId: readPositiveId(body.ikiPegawaiId, "IKI pegawai"),
    jenisPeriode: ensureOneOf(readTrimmedString(body.jenisPeriode || "bulanan").toLowerCase(), targetPeriodTypes, "Jenis periode"),
    periodeKe: readIntegerInRange(body.periodeKe, 1, 12, "Periode ke"),
    tanggalMulai: startDate,
    tanggalSelesai: endDate,
    target: readOptionalNumber(body.target, "Target"),
    milestone: readTrimmedString(body.milestone),
    status: ensureOneOf(readTrimmedString(body.status || "draft").toLowerCase(), targetPeriodStatuses, "Status")
  };
};

const normalizeDialogPayload = (body: Record<string, unknown>) => ({
  periodeId: readPositiveId(body.periodeId, "Periode kinerja"),
  pegawaiId: readPositiveId(body.pegawaiId, "Pegawai"),
  ...
  status: ensureOneOf(readTrimmedString(body.status || "draft").toLowerCase(), ["draft", "diajukan"], "Status")
});
...
export const getIkuSatker = asyncHandler(async (req, res) => {
  ...
  if (req.query.periodeId) {
    ...
    params.push(readPositiveId(req.query.periodeId, "Periode kinerja"));
  }
  if (req.query.status) {
    conditions.push("i.status = ?");
    params.push(ensureOneOf(readTrimmedString(req.query.status).toLowerCase(), planningStatuses, "Status"));
  }
  if (req.query.search) {
  ...
  const [rows] = await pool.query<any[]>(
    `SELECT i.id,
     ...
     ${whereSql}
     ORDER BY p.tahun DESC, p.tanggal_mulai DESC, i.nama_iku ASC`,
    params
  );
...
export const createIkuSatker = asyncHandler(async (req, res) => {
  ...
  if (payload.satuanId) await ensureRecordExists("kinerja_satuan", payload.satuanId, "Satuan");
  if (payload.penanggungJawabPegawaiId) await ensureRecordExists("pegawai", payload.penanggungJawabPegawaiId, "Pegawai penanggung jawab");

  const [result] = await pool.query<ResultSetHeader>(
    `INSERT INTO kinerja_iku_satker (periode_id, sasaran_strategis, nama_iku, definisi, formula, target, satuan_id, bobot, sumber_data, penanggung_jawab_pegawai_id, status)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [payload.periodeId, payload.sasaranStrategis, payload.namaIku, payload.definisi || null, payload.formula || null, payload.target, payload.satuanId, payload.bobot, payload.sumberData || null, payload.penanggungJawabPegawaiId, payload.status]
  );
...
export const updateIkuSatker = asyncHandler(async (req, res) => {
  ...
  await pool.query(
    `UPDATE kinerja_iku_satker
     ...
     WHERE id = ?`,
    [payload.periodeId, payload.sasaranStrategis, payload.namaIku, payload.definisi || null, payload.formula || null, payload.target, payload.satuanId, payload.bobot, payload.sumberData || null, payload.penanggungJawabPegawaiId, payload.status, id]
  );
...
export const deleteIkuSatker = asyncHandler(async (req, res) => {
  await ensurePlanningSchema();
  const id = readPositiveId(req.params.id, "IKU Satker");
  const [result] = await pool.query<ResultSetHeader>(`DELETE FROM kinerja_iku_satker WHERE id = ?`, [id]);
  if (!result.affectedRows) fail("IKU Satker tidak ditemukan", 404);
...
export const getCascadingIku = asyncHandler(async (req, res) => {
  ...
  if (req.query.ikuSatkerId) {
    ...
    params.push(readPositiveId(req.query.ikuSatkerId, "IKU Satker"));
  }
  if (req.query.status) {
  ...
  const [rows] = await pool.query<any[]>(
    `SELECT c.id,
     ...
     ${whereSql}
     ORDER BY i.nama_iku ASC, ind.nama ASC`,
    params
  );
  return sendSuccess(res, rows);
});
...
export const createCascadingIku = asyncHandler(async (req, res) => {
  ...
  if (payload.timKerjaId) await ensureRecordExists("kinerja_tim_kerja", payload.timKerjaId, "Tim kerja");
  if (payload.pegawaiId) await ensureRecordExists("pegawai", payload.pegawaiId, "Pegawai");

  const [result] = await pool.query<ResultSetHeader>(
    `INSERT INTO kinerja_cascading_iku (iku_satker_id, indikator_kinerja_id, tim_kerja_id, pegawai_id, persentase_kontribusi, catatan, status)
     VALUES (?, ?, ?, ?, ?, ?, ?)`,
    [payload.ikuSatkerId, payload.indikatorKinerjaId, payload.timKerjaId, payload.pegawaiId, payload.persentaseKontribusi, payload.catatan || null, payload.status]
  );
...
export const updateCascadingIku = asyncHandler(async (req, res) => {
  ...
  await pool.query(
    `UPDATE kinerja_cascading_iku
     ...
     WHERE id = ?`,
    [payload.ikuSatkerId, payload.indikatorKinerjaId, payload.timKerjaId, payload.pegawaiId, payload.persentaseKontribusi, payload.catatan || null, payload.status, id]
  );
...
export const deleteCascadingIku = asyncHandler(async (req, res) => {
  await ensurePlanningSchema();
  const id = readPositiveId(req.params.id, "Cascading IKU");
  const [result] = await pool.query<ResultSetHeader>(`DELETE FROM kinerja_cascading_iku WHERE id = ?`, [id]);
  if (!result.affectedRows) fail("Cascading IKU tidak ditemukan", 404);
...
export const getIkiPegawai = asyncHandler(async (req, res) => {
  ...
  if (req.query.status) {
    conditions.push("i.status = ?");
    params.push(ensureOneOf(readTrimmedString(req.query.status).toLowerCase(), ikiStatuses, "Status"));
  }
  const whereSql = conditions.length ? `WHERE ${conditions.join(" AND ")}` : "";
  const [rows] = await pool.query<any[]>(
    `SELECT i.id,
     ...
     LEFT JOIN kinerja_satuan s ON s.id = i.satuan_id
     ${whereSql}
     ORDER BY p.tahun DESC, pg.nama_lengkap ASC, i.nama_iki ASC`,
    params
  );
  return sendSuccess(res, rows);
});
...
export const createIkiPegawai = asyncHandler(async (req, res) => {
  ...
  if (payload.timKerjaId) await ensureRecordExists("kinerja_tim_kerja", payload.timKerjaId, "Tim kerja");
  if (payload.indikatorKinerjaId) await ensureRecordExists("indikator_kinerja", payload.indikatorKinerjaId, "Indikator kinerja");
  if (payload.satuanId) await ensureRecordExists("kinerja_satuan", payload.satuanId, "Satuan");
  const [result] = await pool.query<ResultSetHeader>(
    `INSERT INTO kinerja_iki_pegawai (periode_id, pegawai_id, tim_kerja_id, indikator_kinerja_id, nama_iki, target, satuan_id, bobot, metode_ukur, sumber_bukti, status)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [payload.periodeId, payload.pegawaiId, payload.timKerjaId, payload.indikatorKinerjaId, payload.namaIki, payload.target, payload.satuanId, payload.bobot, payload.metodeUkur || null, payload.sumberBukti || null, payload.status]
  );
...
export const updateIkiPegawai = asyncHandler(async (req, res) => {
  ...
  await pool.query(
    `UPDATE kinerja_iki_pegawai
     ...
     WHERE id = ?`,
    [payload.periodeId, payload.pegawaiId, payload.timKerjaId, payload.indikatorKinerjaId, payload.namaIki, payload.target, payload.satuanId, payload.bobot, payload.metodeUkur || null, payload.sumberBukti || null, payload.status, id]
  );
  return sendSuccess(res, null, "IKI pegawai berhasil diperbarui");
});

const updateIkiStatus = async (id: number, nextStatus: (typeof ikiStatuses)[number], allowedCurrent: string[], successMessage: string) => {
  const [rows] = await pool.query<any[]>(`SELECT id, status FROM kinerja_iki_pegawai WHERE id = ? LIMIT 1`, [id]);
  ...
  if (!allowedCurrent.includes(currentStatus)) {
    fail(`IKI pegawai tidak dapat diproses dari status ${currentStatus}`, 400);
  }
  await pool.query(`UPDATE kinerja_iki_pegawai SET status = ? WHERE id = ?`, [nextStatus, id]);
...
export const getTargetPeriodik = asyncHandler(async (req, res) => {
  ...
  const [rows] = await pool.query<any[]>(
    `SELECT t.id,
     ...
     INNER JOIN pegawai pg ON pg.id = i.pegawai_id
     ${whereSql}
     ORDER BY pg.nama_lengkap ASC, i.nama_iki ASC, t.jenis_periode ASC, t.periode_ke ASC`,
    params
  );
  return sendSuccess(res, rows);
});
...
export const createTargetPeriodik = asyncHandler(async (req, res) => {
  ...
  const [result] = await pool.query<ResultSetHeader>(
    `INSERT INTO kinerja_target_periodik (iki_pegawai_id, jenis_periode, periode_ke, tanggal_mulai, tanggal_selesai, target, milestone, status)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    [payload.ikiPegawaiId, payload.jenisPeriode, payload.periodeKe, payload.tanggalMulai, payload.tanggalSelesai, payload.target, payload.milestone || null, payload.status]
  );
...
export const updateTargetPeriodik = asyncHandler(async (req, res) => {
  ...
  await pool.query(
    `UPDATE kinerja_target_periodik
     ...
     WHERE id = ?`,
    [payload.ikiPegawaiId, payload.jenisPeriode, payload.periodeKe, payload.tanggalMulai, payload.tanggalSelesai, payload.target, payload.milestone || null, payload.status, id]
  );
  return sendSuccess(res, null, "Target periodik berhasil diperbarui");
});

const buildAutoPeriods = (year: number, jenis: (typeof targetPeriodTypes)[number]) => {
  if (jenis === "semester") {
    return [
...
export const generateTargetPeriodik = asyncHandler(async (req, res) => {
  ...
  const jenisPeriode = ensureOneOf(readTrimmedString(req.body?.jenisPeriode || "bulanan").toLowerCase(), targetPeriodTypes, "Jenis periode");
  const [[ikiRow]] = await pool.query<any[]>(
    `SELECT i.id, i.target, p.tahun
     FROM kinerja_iki_pegawai i
     INNER JOIN kinerja_periode p ON p.id = i.periode_id
     WHERE i.id = ? LIMIT 1`,
    [ikiPegawaiId]
  );
  if (!ikiRow) fail("IKI pegawai tidak ditemukan", 404);

  const [existing] = await pool.query<any[]>(
    `SELECT id FROM kinerja_target_periodik WHERE iki_pegawai_id = ? AND jenis_periode = ? LIMIT 1`,
    [ikiPegawaiId, jenisPeriode]
  );
  if (existing.length) {
    fail("Target periodik untuk IKI dan jenis periode ini sudah ada", 400);
  }
  ...
  await pool.query(
    `INSERT INTO kinerja_target_periodik (iki_pegawai_id, jenis_periode, periode_ke, tanggal_mulai, tanggal_selesai, target, milestone, status)
     VALUES ${periods.map(() => `(?, ?, ?, ?, ?, ?, ?, 'draft')`).join(", ")}`,
    periods.flatMap((period) => [ikiPegawaiId, jenisPeriode, period.periodeKe, period.tanggalMulai, period.tanggalSelesai, splitTarget, `Auto generate ${jenisPeriode} ${period.periodeKe}`])
  );
  return sendSuccess(res, { generated: periods.length }, "Target periodik otomatis berhasil dibuat", 201);
});
...
export const getDialogAwal = asyncHandler(async (req, res) => {
  ...
  const [rows] = await pool.query<any[]>(
    `SELECT d.id,
     ...
     ${whereSql}
     ORDER BY p.tahun DESC, pg.nama_lengkap ASC`,
    params
  );
  return sendSuccess(res, rows);
});
...
export const createDialogAwal = asyncHandler(async (req, res) => {
  ...
  const [result] = await pool.query<ResultSetHeader>(
    `INSERT INTO kinerja_dialog_awal (periode_id, pegawai_id, penilai_pegawai_id, ringkasan_target, ekspektasi_hasil, ekspektasi_perilaku, risiko, dukungan_dibutuhkan, catatan_dialog, status)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [payload.periodeId, payload.pegawaiId, payload.penilaiPegawaiId, payload.ringkasanTarget || null, payload.ekspektasiHasil || null, payload.ekspektasiPerilaku || null, payload.risiko || null, payload.dukunganDibutuhkan || null, payload.catatanDialog || null, payload.status]
  );
...
export const updateDialogAwal = asyncHandler(async (req, res) => {
  ...
  await pool.query(
    `UPDATE kinerja_dialog_awal
     ...
     WHERE id = ?`,
    [payload.periodeId, payload.pegawaiId, payload.penilaiPegawaiId, payload.ringkasanTarget || null, payload.ekspektasiHasil || null, payload.ekspektasiPerilaku || null, payload.risiko || null, payload.dukunganDibutuhkan || null, payload.catatanDialog || null, payload.status, id]
  );
...
export const approveDialogAwal = asyncHandler(async (req, res) => {
  ...
  if (!["draft", "diajukan"].includes(currentStatus)) {
    fail(`Dialog awal tidak dapat disetujui dari status ${currentStatus}`, 400);
  }
  await pool.query(`UPDATE kinerja_dialog_awal SET status = 'disetujui' WHERE id = ?`, [id]);
