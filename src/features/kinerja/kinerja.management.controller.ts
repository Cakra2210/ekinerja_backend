...
import { activityStatusSql, getTimerDurationSecondsSql } from "./kinerja.timer.helper";
import {
  ensureOneOf,
  ensureRequired,
  readDateString,
...
const FEEDBACK_TYPES = ["apresiasi", "koreksi", "pembinaan"] as const;
const READ_STATUSES = ["baru", "dibaca"] as const;
const TARGET_CHANGE_STATUSES = ["diajukan", "disetujui", "ditolak"] as const;
const RISK_LEVELS = ["rendah", "sedang", "tinggi"] as const;
...
const readOptionalMonth = (value: unknown) => {
  if (value === undefined || value === null || value === "") return null;
  return readIntegerInRange(value, 1, 12, "Bulan");
};

const readOptionalYear = (value: unknown) => {
  if (value === undefined || value === null || value === "") return null;
  return readIntegerInRange(value, 2020, 2100, "Tahun");
};
...
const ensureIkiExists = async (ikiId: number) => {
  const [rows] = await pool.query<any[]>(`SELECT id FROM kinerja_iki_pegawai WHERE id = ? LIMIT 1`, [ikiId]);
  if (!rows.length) fail("IKI pegawai tidak ditemukan", 404);
};

const ensureFeedbackExists = async (feedbackId: number) => {
  const [rows] = await pool.query<any[]>(`SELECT id FROM kinerja_umpan_balik WHERE id = ? LIMIT 1`, [feedbackId]);
  if (!rows.length) fail("Umpan balik tidak ditemukan", 404);
};

const ensureTargetChangeExists = async (changeId: number) => {
  const [rows] = await pool.query<any[]>(`SELECT id FROM kinerja_perubahan_target WHERE id = ? LIMIT 1`, [changeId]);
  if (!rows.length) fail("Perubahan target tidak ditemukan", 404);
};

const ensureNotificationExists = async (notificationId: number) => {
  const [rows] = await pool.query<any[]>(`SELECT id FROM kinerja_notifikasi WHERE id = ? LIMIT 1`, [notificationId]);
  if (!rows.length) fail("Notifikasi tidak ditemukan", 404);
};
...
const ensureManagementSchema = async () => {
  if (managementSchemaReady) return;
...
const normalizeFeedbackPayload = (body: Record<string, unknown>) => ({
  periodeId: readOptionalPositiveId(body.periodeId, "Periode kinerja"),
  pegawaiId: readPositiveId(body.pegawaiId, "Pegawai penerima"),
  pemberiFeedbackId: readPositiveId(body.pemberiFeedbackId, "Pemberi umpan balik"),
  ...
  feedbackType: ensureOneOf(
    readTrimmedString(body.feedbackType || "koreksi").toLowerCase(),
    FEEDBACK_TYPES,
    "Jenis umpan balik"
  ),
  content: ensureRequired(readTrimmedString(body.content), "Isi umpan balik wajib diisi"),
  feedbackDate: readDateString(body.feedbackDate, "Tanggal umpan balik"),
  readStatus: ensureOneOf(
    readTrimmedString(body.readStatus || "baru").toLowerCase(),
    READ_STATUSES,
    "Status baca"
  )
});

const normalizeTargetChangePayload = (body: Record<string, unknown>, fallbackEmployeeId: number | null) => ({
  ikiPegawaiId: readPositiveId(body.ikiPegawaiId, "IKI pegawai"),
  oldTarget: readNonNegativeNumber(body.oldTarget, "Target lama", 0),
  newTarget: readNonNegativeNumber(body.newTarget, "Target baru", 0),
  reason: ensureRequired(readTrimmedString(body.reason), "Alasan perubahan target wajib diisi"),
  impactNote: readTrimmedString(body.impactNote),
  requestedBy: readOptionalPositiveId(body.requestedBy, "Pengaju") || fallbackEmployeeId || 0,
  approvalNote: readTrimmedString(body.approvalNote)
});

const buildFeedbackRecord = (row: any) => ({
  id: Number(row.id),
...
const buildTargetChangeRecord = (row: any) => ({
  id: Number(row.id),
...
const buildNotificationRecord = (row: any) => ({
  id: Number(row.id),
  pegawaiId: Number(row.pegawaiId),
  pegawaiName: String(row.pegawaiName || "-"),
...
const buildRiskQuery = (filters: {
  ...
  const realizationConditions: string[] = ["1 = 1"];
  const realizationParams: any[] = [];
  if (filters.month) {
  ...
  const feedbackConditions: string[] = ["1 = 1"];
  ...
  const changeConditions: string[] = ["1 = 1"];
...
export const getKinerjaAnalyticsFollowUpDirect = asyncHandler(async (req: AuthenticatedRequest, res) => {
  ...
  const workloads = employees
    .map((employee) => ({
      ...
      workloadLabel:
        employee.highRisk > 0 || employee.pendingTargetChanges > 0
          ? "Prioritas tinggi"
...
export const getKinerjaBerandaDashboard = asyncHandler(async (req: AuthenticatedRequest, res) => {
  await ensureManagementSchema();
  ...
  const ikiConditions: string[] = ["1 = 1"];
  const ikiParams: any[] = [];
  ...
  const [summaryRows] = await pool.query<any[]>(
    `SELECT
     ...
     WHERE ${ikiConditions.join(" AND ")}`,
    [...realizationParams, ...ikiParams]
  );
  ...
  const [[feedbackSummary]] = await pool.query<any[]>(
    `SELECT
  ...
  const targetChangeConditions: string[] = ["1 = 1"];
  const targetChangeParams: any[] = [];
  ...
  const [[targetChangeSummary]] = await pool.query<any[]>(
    `SELECT
     ...
     WHERE ${targetChangeConditions.join(" AND ")}`,
    targetChangeParams
  );

  const notificationConditions: string[] = ["1 = 1"];
  ...
  const [[notificationSummary]] = await pool.query<any[]>(
    `SELECT
  ...
  const [riskRows] = await pool.query<any[]>(
    `SELECT
        iki.id,
        CASE
          WHEN COALESCE(ch.pending_changes, 0) > 0
  ...
  const [yearRows] = await pool.query<any[]>(
    `SELECT DISTINCT tahun AS value FROM kinerja_periode WHERE tahun IS NOT NULL
     UNION
     SELECT DISTINCT YEAR(tanggal_lapor) AS value FROM kinerja_realisasi_indikator WHERE tanggal_lapor IS NOT NULL
     ORDER BY value DESC`
  );

  const [teamLookupRows] = await pool.query<any[]>(
    `SELECT id, nama_tim AS name FROM kinerja_tim_kerja ORDER BY nama_tim ASC`
  );

  const [employeeLookupRows] = await pool.query<any[]>(
    `SELECT DISTINCT p.id, p.nama_lengkap AS name
     FROM pegawai p
     INNER JOIN kinerja_iki_pegawai iki ON iki.pegawai_id = p.id
     ORDER BY p.nama_lengkap ASC`
  );

  const [[employeeProfileRow]] = await pool.query<any[]>(
    `SELECT
  ...
  const [[positionFulfillmentRow]] = await pool.query<any[]>(
    `SELECT
  ...
  const averageEducationLabel = educationLevels.reduce(
    (closest, item) =>
      Math.abs(item.score - averageEducationScore) < Math.abs(closest.score - averageEducationScore) ? item : closest,
    educationLevels[0]
  ).label;
...
export const getKinerjaDashboardOverview = asyncHandler(async (req: AuthenticatedRequest, res) => {
  await ensureManagementSchema();

  const periodId = req.query.periodId ? readPositiveId(req.query.periodId, "Periode kinerja") : null;
  const year = readOptionalYear(req.query.year);
  const month = readOptionalMonth(req.query.month);
  const teamId = req.query.teamId ? readPositiveId(req.query.teamId, "Tim kerja") : null;
  const employeeId = req.query.employeeId ? readPositiveId(req.query.employeeId, "Pegawai") : null;
  ...
  const [[targetChangeSummary]] = await pool.query<any[]>(
    `SELECT COUNT(*) AS totalTargetChanges,
  ...
  const [riskRows] = await pool.query<any[]>(riskQuery.sql, riskQuery.params);
  ...
  const notificationConditions: string[] = ["(? = 1 OR n.pegawai_id = ?)"];
  const notificationParams: any[] = [isPrivileged ? 1 : 0, currentEmployeeId || 0];
  if (employeeId) {
    notificationConditions.push("n.pegawai_id = ?");
    notificationParams.push(employeeId);
  }
  ...
  if (dashboardLooksEmpty) {
    ...
    if (Number(rawSummary?.totalIki || 0) > 0) {
      const [[rawFeedbackSummary]] = await pool.query<any[]>(
        `SELECT COUNT(*) AS totalFeedback,
                SUM(CASE WHEN status_baca = 'baru' THEN 1 ELSE 0 END) AS unreadFeedback
         FROM kinerja_umpan_balik`
      );

      const [[rawTargetChangeSummary]] = await pool.query<any[]>(
        `SELECT COUNT(*) AS totalTargetChanges,
                SUM(CASE WHEN pt.status = 'diajukan' THEN 1 ELSE 0 END) AS pendingTargetChanges
         FROM kinerja_perubahan_target pt`
      );

      const [[rawNotificationSummary]] = await pool.query<any[]>(
        `SELECT COUNT(*) AS totalNotifications,
                SUM(CASE WHEN status_baca = 'baru' THEN 1 ELSE 0 END) AS unreadNotifications
         FROM kinerja_notifikasi`
      );

      const [rawRiskRows] = await pool.query<any[]>(
        `SELECT
            CASE
              WHEN COALESCE(ch.pending_changes, 0) > 0
                OR (CASE WHEN COALESCE(iki.target, 0) > 0 THEN (COALESCE(real_data.total_realisasi, 0) / iki.target) * 100 ELSE 0 END) < 50
      ...
      const [rawTeamRows] = await pool.query<any[]>(
        `SELECT
            tk.id,
            tk.nama_tim AS teamName,
            COUNT(iki.id) AS totalIndicators,
      ...
      const [rawNotificationRows] = await pool.query<any[]>(
        `SELECT n.id,
                n.pegawai_id AS pegawaiId,
                p.nama_lengkap AS pegawaiName,
                n.jenis_notifikasi AS notificationType,
...
export const getKinerjaDashboardDiagnostics = asyncHandler(async (_req: AuthenticatedRequest, res) => {
  const tables = [
    "pegawai",
    "kinerja_periode",
    "kinerja_tim_kerja",
  ...
  const [[databaseRow]] = await pool.query<any[]>(
    `SELECT DATABASE() AS databaseName, @@hostname AS databaseHost, @@port AS databasePort`
  );
  ...
  for (const table of tables) {
    try {
      const [[row]] = await pool.query<any[]>(`SELECT COUNT(*) AS total FROM \`${table}\``);
  ...
  const [sampleIki] = await pool.query<any[]>(
    `SELECT iki.id,
            p.nama_lengkap AS pegawaiName,
            COALESCE(tk.nama_tim, '-') AS teamName,
            iki.nama_iki AS ikiName,
...
export const getKinerjaRiskMonitoringDirect = asyncHandler(async (req: AuthenticatedRequest, res) => {
  await ensureManagementSchema();
  ...
  const riskLevel = req.query.riskLevel
    ? ensureOneOf(readTrimmedString(req.query.riskLevel).toLowerCase(), RISK_LEVELS, "Level risiko")
    : null;
...
export const getKinerjaRiskMonitoring = asyncHandler(async (req, res) => {
  await ensureManagementSchema();
...
export const getKinerjaFeedbacks = asyncHandler(async (req, res) => {
  await ensureManagementSchema();

  const employeeId = req.query.employeeId ? readPositiveId(req.query.employeeId, "Pegawai") : null;
  const feedbackType = req.query.feedbackType
    ? ensureOneOf(readTrimmedString(req.query.feedbackType).toLowerCase(), FEEDBACK_TYPES, "Jenis umpan balik")
    : null;
  const readStatus = req.query.readStatus
    ? ensureOneOf(readTrimmedString(req.query.readStatus).toLowerCase(), READ_STATUSES, "Status baca")
    : null;
  const search = readTrimmedString(req.query.search);
  ...
  const [rows] = await pool.query<any[]>(
    `SELECT f.id,
     ...
     WHERE ${conditions.join(" AND ")}
     ORDER BY f.tanggal_feedback DESC, f.dibuat_pada DESC`,
    params
  );

  return sendSuccess(res, rows.map(buildFeedbackRecord));
});

export const createKinerjaFeedback = asyncHandler(async (req: AuthenticatedRequest, res) => {
  await ensureManagementSchema();
  ...
  const [result] = await pool.query<ResultSetHeader>(
    `INSERT INTO kinerja_umpan_balik
  ...
  return sendSuccess(res, { id: result.insertId }, "Umpan balik berhasil ditambahkan", 201);
});

export const updateKinerjaFeedback = asyncHandler(async (_req: AuthenticatedRequest, res) => {
  await ensureManagementSchema();
  const id = readPositiveId(_req.params.id, "Umpan balik");
  ...
  return sendSuccess(res, null, "Umpan balik berhasil diperbarui");
});

export const markKinerjaFeedbackAsRead = asyncHandler(async (req, res) => {
  await ensureManagementSchema();
  const id = readPositiveId(req.params.id, "Umpan balik");
  await ensureFeedbackExists(id);

  await pool.query(`UPDATE kinerja_umpan_balik SET status_baca = 'dibaca' WHERE id = ?`, [id]);
  return sendSuccess(res, null, "Umpan balik ditandai sudah dibaca");
});

export const getKinerjaTargetChanges = asyncHandler(async (req, res) => {
  await ensureManagementSchema();

  const status = req.query.status
    ? ensureOneOf(readTrimmedString(req.query.status).toLowerCase(), TARGET_CHANGE_STATUSES, "Status perubahan target")
    : null;
  const employeeId = req.query.employeeId ? readPositiveId(req.query.employeeId, "Pegawai") : null;
  const search = readTrimmedString(req.query.search);
  ...
  const [rows] = await pool.query<any[]>(
    `SELECT pt.id,
     ...
     LEFT JOIN kinerja_periode kp ON kp.id = iki.periode_id
     WHERE ${conditions.join(" AND ")}
     ORDER BY pt.diajukan_pada DESC`,
    params
  );

  return sendSuccess(res, rows.map(buildTargetChangeRecord));
});

export const createKinerjaTargetChange = asyncHandler(async (req: AuthenticatedRequest, res) => {
  await ensureManagementSchema();

  const currentEmployeeId = req.user?.employeeId || null;
  const payload = normalizeTargetChangePayload(req.body || {}, currentEmployeeId);

  if (!payload.requestedBy) fail("Pengaju perubahan target tidak ditemukan", 400);
  await ensureIkiExists(payload.ikiPegawaiId);
  await ensureEmployeeExists(payload.requestedBy);

  const [[ikiRow]] = await pool.query<any[]>(
    `SELECT iki.pegawai_id AS pegawaiId,
            iki.nama_iki AS ikiName
     FROM kinerja_iki_pegawai iki
     WHERE iki.id = ? LIMIT 1`,
    [payload.ikiPegawaiId]
  );

  const [result] = await pool.query<ResultSetHeader>(
    `INSERT INTO kinerja_perubahan_target
  ...
  return sendSuccess(res, { id: result.insertId }, "Usulan perubahan target berhasil ditambahkan", 201);
});

export const approveKinerjaTargetChange = asyncHandler(async (req: AuthenticatedRequest, res) => {
  await ensureManagementSchema();
  const id = readPositiveId(req.params.id, "Perubahan target");
  await ensureTargetChangeExists(id);
  const approverId = req.user?.employeeId || null;
  const approvalNote = readTrimmedString(req.body?.approvalNote);

  const [[changeRow]] = await pool.query<any[]>(
    `SELECT pt.iki_pegawai_id AS ikiPegawaiId,
            pt.target_baru AS newTarget,
            iki.pegawai_id AS pegawaiId,
            iki.nama_iki AS ikiName
     FROM kinerja_perubahan_target pt
  ...
  if (!changeRow) fail("Perubahan target tidak ditemukan", 404);

  await pool.query(
    `UPDATE kinerja_perubahan_target
     SET status = 'disetujui', diproses_oleh = ?, diproses_pada = NOW(), catatan_persetujuan = ?
     WHERE id = ?`,
    [approverId, approvalNote || null, id]
  );
  ...
  await pool.query(
    `UPDATE kinerja_iki_pegawai
     SET target = ?
     WHERE id = ?`,
    [changeRow.newTarget, changeRow.ikiPegawaiId]
  );
  ...
  return sendSuccess(res, null, "Usulan perubahan target disetujui");
});

export const rejectKinerjaTargetChange = asyncHandler(async (req: AuthenticatedRequest, res) => {
  await ensureManagementSchema();
  const id = readPositiveId(req.params.id, "Perubahan target");
  await ensureTargetChangeExists(id);
  const approverId = req.user?.employeeId || null;
  ...
  return sendSuccess(res, null, "Usulan perubahan target ditolak");
});

export const getKinerjaNotifications = asyncHandler(async (req: AuthenticatedRequest, res) => {
  await ensureManagementSchema();
  const currentEmployeeId = req.user?.employeeId || null;
  const isPrivileged = ["super_admin", "admin_satker", "kepala_satker", "kasubbag_umum", "ketua_tim", "pejabat_penilai", "reviewer"].includes(String(req.user?.role || ""));
  ...
  return sendSuccess(res, rows.map(buildNotificationRecord));
});

export const markKinerjaNotificationAsRead = asyncHandler(async (req, res) => {
  await ensureManagementSchema();
  const id = readPositiveId(req.params.id, "Notifikasi");
  await ensureNotificationExists(id);

  await pool.query(`UPDATE kinerja_notifikasi SET status_baca = 'dibaca' WHERE id = ?`, [id]);
  return sendSuccess(res, null, "Notifikasi ditandai sudah dibaca");
});

export const markAllKinerjaNotificationsAsRead = asyncHandler(async (req: AuthenticatedRequest, res) => {
  await ensureManagementSchema();
  const currentEmployeeId = req.user?.employeeId || null;
  const isPrivileged = ["super_admin", "admin_satker", "kepala_satker", "kasubbag_umum", "ketua_tim", "pejabat_penilai", "reviewer"].includes(String(req.user?.role || ""));
  const employeeId = req.body?.employeeId ? readPositiveId(req.body.employeeId, "Pegawai") : currentEmployeeId;

  if (!employeeId) fail("Pegawai notifikasi tidak ditemukan", 400);
  if (!isPrivileged && employeeId !== currentEmployeeId) fail("Anda tidak memiliki hak akses untuk tindakan ini", 403);

  await pool.query(`UPDATE kinerja_notifikasi SET status_baca = 'dibaca' WHERE pegawai_id = ?`, [employeeId]);
  return sendSuccess(res, null, "Seluruh notifikasi ditandai sudah dibaca");
});
...
const toOptionalDashboardDate = (value: unknown) => {
  if (value === undefined || value === null || value === "") return null;
  return readOptionalDateString(value, "Tanggal dashboard");
};

const buildDailyWorkScope = (
  req: AuthenticatedRequest,
  filters: { unitId?: number | null; teamId?: number | null } = {},
  aliases: { employee?: string; team?: string; assignment?: string } = {}
) => {
...
const readDailyWorkFilters = (req: AuthenticatedRequest) => ({
...
export const getKinerjaDailyWorkDashboard = asyncHandler(async (req: AuthenticatedRequest, res) => {
  ...
  const [currentEmployeeRows] = await pool.query<any[]>(
    `SELECT
        p.id,
  ...
  const [unitRows] = await pool.query<any[]>(
    `SELECT id, nama AS name FROM departemen ORDER BY nama ASC`
  );

  const [teamRows] = await pool.query<any[]>(
    `SELECT id, nama_tim AS name FROM kinerja_tim_kerja WHERE status = 'aktif' ORDER BY nama_tim ASC`
  );

  const [activeActivityRows] = await pool.query<any[]>(
    `SELECT
        l.id,
        ...
        IF(a.target_selesai IS NOT NULL AND a.target_selesai < CURDATE() AND ${activityStatusSql("l")} IN ('berjalan','jeda'), 1, 0) AS isOverdue
     ...
     WHERE l.pegawai_id = ?
       AND ${activityStatusSql("l")} IN ('berjalan','jeda')
     ORDER BY COALESCE(l.last_activity_at, l.started_at, l.diperbarui_pada, l.dibuat_pada) DESC
     LIMIT 1`,
    [currentEmployeeId || -1]
  );

  const [assignmentSummaryRows] = await pool.query<any[]>(
    `SELECT
     ...
     WHERE a.pegawai_id = ?`,
    [selectedDateParam, selectedDateParam, selectedDateParam, selectedDateParam, currentEmployeeId || -1]
  );

  const [priorityAssignmentRows] = await pool.query<any[]>(
    `SELECT
        a.id,
        a.judul AS title,
        a.status,
     ...
     LIMIT 8`,
    [selectedDateParam, currentEmployeeId || -1]
  );

  const [recentActivityRows] = await pool.query<any[]>(
    `SELECT
        l.id,
        COALESCE(NULLIF(l.uraian_kegiatan, ''), a.judul, 'Aktivitas tanpa uraian') AS activityTitle,
        COALESCE(a.judul, 'Tanpa penugasan') AS assignmentTitle,
  ...
  const [realtimeSummaryRows] = await pool.query<any[]>(
    `SELECT
        COUNT(CASE WHEN ${activityStatusSql("l")} = 'berjalan' THEN 1 END) AS running,
        COUNT(CASE WHEN ${activityStatusSql("l")} = 'jeda' THEN 1 END) AS paused,
        COUNT(CASE WHEN ${activityStatusSql("l")} = 'selesai' THEN 1 END) AS finishedToday,
        COUNT(CASE WHEN a.target_selesai IS NOT NULL AND a.target_selesai < COALESCE(?, CURDATE()) AND ${activityStatusSql("l")} IN ('berjalan','jeda') THEN 1 END) AS overdue
     FROM kinerja_logbook l
     ...
     WHERE (${scope.sql})
       ...
       AND ${activityStatusSql("l")} IN ('berjalan','jeda','selesai')`,
    [selectedDateParam, ...scope.params, selectedDateParam, selectedDateParam]
  );

  const [attentionRows] = await pool.query<any[]>(
    `SELECT
        l.id,
        p.nama_lengkap AS employeeName,
        ...
        DATE_FORMAT(a.target_selesai, '%Y-%m-%d') AS dueDate,
        CASE
          WHEN a.target_selesai IS NOT NULL AND a.target_selesai < COALESCE(?, CURDATE()) AND ${activityStatusSql("l")} IN ('berjalan','jeda') THEN 'Melewati batas akhir penugasan'
          WHEN ${activityStatusSql("l")} = 'berjalan' AND ${getTimerDurationSecondsSql("l")} >= 14400 THEN 'Aktivitas berjalan lebih dari 4 jam'
          WHEN ${activityStatusSql("l")} = 'jeda' AND l.paused_at IS NOT NULL AND TIMESTAMPDIFF(SECOND, l.paused_at, NOW()) >= 3600 THEN 'Aktivitas jeda lebih dari 1 jam'
          ELSE 'Perlu ditinjau'
        END AS reason,
        IF(a.target_selesai IS NOT NULL AND a.target_selesai < COALESCE(?, CURDATE()) AND ${activityStatusSql("l")} IN ('berjalan','jeda'), 1, 0) AS isOverdue
     FROM kinerja_logbook l
     INNER JOIN pegawai p ON p.id = l.pegawai_id
     LEFT JOIN departemen d ON d.id = p.departemen_id
     LEFT JOIN kinerja_assignment a ON a.id = l.assignment_id
     ...
     WHERE (${scope.sql})
       ...
       AND (
         ...
         OR (${activityStatusSql("l")} = 'berjalan' AND ${getTimerDurationSecondsSql("l")} >= 14400)
         OR (${activityStatusSql("l")} = 'jeda' AND l.paused_at IS NOT NULL AND TIMESTAMPDIFF(SECOND, l.paused_at, NOW()) >= 3600)
       )
     ORDER BY isOverdue DESC, activeDurationSeconds DESC, COALESCE(l.last_activity_at, l.started_at, l.diperbarui_pada) DESC
     LIMIT 8`,
    [selectedDateParam, selectedDateParam, ...scope.params, selectedDateParam]
  );

  const [todayActivityRows] = await pool.query<any[]>(
    `SELECT
        l.id,
        p.id AS employeeId,
        p.nama_lengkap AS employeeName,
     ...
     WHERE (${scope.sql})
       ...
       AND ${activityStatusSql("l")} IN ('berjalan','jeda','selesai')
     ORDER BY isOverdue DESC,
              FIELD(${activityStatusSql("l")}, 'berjalan', 'jeda', 'selesai'),
              COALESCE(l.last_activity_at, l.started_at, l.diperbarui_pada, l.dibuat_pada) DESC
     LIMIT 10`,
    [selectedDateParam, ...scope.params, selectedDateParam, selectedDateParam]
  );

  const [statusTrendRows] = await pool.query<any[]>(
    `SELECT
        ${activityStatusSql("l")} AS activityStatus,
        COUNT(*) AS total
     FROM kinerja_logbook l
     ...
     WHERE (${scope.sql})
       ...
       AND ${activityStatusSql("l")} IN ('berjalan','jeda','selesai')
     GROUP BY ${activityStatusSql("l")}`,
    [...scope.params, selectedDateParam, selectedDateParam]
  );
  ...
  statusTrendRows.forEach((row) => {
    if (row.activityStatus === "berjalan") statusDistribution.running = Number(row.total || 0);
    if (row.activityStatus === "jeda") statusDistribution.paused = Number(row.total || 0);
    if (row.activityStatus === "selesai") statusDistribution.finished = Number(row.total || 0);
  });

  const [selectedDateRows] = await pool.query<any[]>(
    `SELECT DATE_FORMAT(COALESCE(?, CURDATE()), '%Y-%m-%d') AS selectedDate,
            DATE_FORMAT(NOW(), '%Y-%m-%dT%H:%i:%s') AS serverNow`,
    [selectedDateParam]
  );
