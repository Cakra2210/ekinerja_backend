...
const parseQueryText = (value: unknown, fallback = "") => {
  const rawValue = Array.isArray(value) ? value[0] : value;
...
const normalizeDateOnlyText = (value: unknown, fallback = "") => {
...
const toDateOnlyText = (value: unknown) => normalizeDateOnlyText(value, "");
...
const normalizeActivityStatus = (status: unknown, statusAktivitas?: unknown) => {
  ...
  if (candidate === "dijeda") return "jeda";
  if (["berjalan", "jeda", "selesai"].includes(candidate)) return candidate;
  return "belum_mulai";
};
...
type DeadlineMeta = {
  ...
  status: "overdue" | "today" | "soon" | "safe" | "done" | "no_deadline";
};

const parseDateOnlyAsUtc = (value: unknown) => {
...
const differenceInCalendarDays = (targetDate: unknown, referenceDate: unknown) => {
...
const isManagerRole = (role?: string | null) =>
  ["super_admin", "admin_satker", "kepala_satker", "kasubbag_umum", "pejabat_penilai", "reviewer"].includes(role || "");

const createPlaceholders = (length: number) => Array.from({ length }, () => "?").join(", ");

const getScopedEmployees = async (req: AuthenticatedRequest) => {
  const user = req.user;
  ...
  if (isManagerRole(role)) {
    const [rows] = await pool.query<any[]>(`${baseSelect} ORDER BY p.nama_lengkap ASC`);
    return rows;
  }
  ...
  const [rows] = await pool.query<any[]>(`${baseSelect} AND p.id = ? ORDER BY p.nama_lengkap ASC`, [employeeId]);
  return rows;
};

const getActivePeriod = async (date: string) => {
  const [rows] = await pool.query<any[]>(
    `SELECT id, tahun, nama_periode, jenis_periode, tanggal_mulai::text AS tanggal_mulai, tanggal_selesai::text AS tanggal_selesai, status
...
const resolveDashboardDate = async (requestedDate: string) => {
  ...
  if (periodRows.length) return requested;
...
const getAssignmentRows = async (employeeIds: number[], date: string) => {
  ...
  const [rows] = await pool.query<any[]>(
    `SELECT a.id,
     ...
     ORDER BY is_overdue DESC, is_due_soon DESC, a.target_selesai ASC NULLS LAST, a.diperbarui_pada DESC`,
    [date, date, date, ...employeeIds]
  );
  return rows;
};

const getLogbookRows = async (employeeIds: number[], date: string) => {
  if (!employeeIds.length) return [];
  const placeholders = createPlaceholders(employeeIds.length);
  const [rows] = await pool.query<any[]>(
    `SELECT l.id,
...
const getWeeklyActivity = async (employeeIds: number[], date: string) => {
  if (!employeeIds.length) return [];
  const placeholders = createPlaceholders(employeeIds.length);
  const [rows] = await pool.query<any[]>(
    `SELECT l.tanggal_kegiatan::text AS activity_date,
     ...
     ORDER BY l.tanggal_kegiatan ASC`,
...
const getFeedbackSummary = async (employeeIds: number[]) => {
  if (!employeeIds.length) return { unread: 0, latest: [] as any[] };
  const placeholders = createPlaceholders(employeeIds.length);
  const [summaryRows] = await pool.query<any[]>(
    `SELECT COUNT(*) AS unread_count
     FROM kinerja_umpan_balik
     WHERE pegawai_id IN (${placeholders})
       AND status_baca = 'baru'`,
    employeeIds
  );
...
const getTargetSummary = async (employeeIds: number[], date: string) => {
  ...
  const placeholders = createPlaceholders(employeeIds.length);
  const [rows] = await pool.query<any[]>(
    `SELECT COUNT(DISTINCT i.id) AS total_iki,
     ...
     WHERE i.pegawai_id IN (${placeholders})`,
    [date, date, ...employeeIds]
  );
...
const getTeamSummary = async (employeeIds: number[], role: string, employeeId: number) => {
  ...
  const params: unknown[] = restrictToLeader ? [employeeId] : employeeIds;
...
const buildCurrentActivity = (logbookRows: any[], employeeId: number) => {
...
export const getCommandCenterDashboard = asyncHandler(async (req: Request, res) => {
